"""Canonical Prompt Runtime 管理路由。

`/prompt-v2/*` 旧路径保留为兼容别名，主路径使用 `/prompt/*`。
"""
...
def create_prompt_v2_template(
    ...
    try:
        ...
        raise HTTPException(400, str(e))
    audit_request(db, request, "create_prompt_template", "prompt", result["template_key"], result)
    return result


@router.get("/prompt/templates/{template_key:path}")
...
def get_prompt_v2_template(template_key: str, db: Session = Depends(get_db), _auth=Depends(verify_admin)):
    ...
    try:
        ...
        raise HTTPException(404, "Prompt 模板不存在")
    except Exception as e:
        raise HTTPException(400, str(e))


@router.put("/prompt/templates/{template_key:path}")
...
def save_prompt_v2_template(
    ...
    try:
        ...
        raise HTTPException(400, str(e))
    audit_request(db, request, "save_prompt_template", "prompt", template_key, result)
    return result


@router.delete("/prompt/templates/{template_key:path}")
...
def delete_prompt_v2_template(
    ...
    try:
        ...
        raise HTTPException(400, str(e))
    audit_request(db, request, "delete_prompt_template", "prompt", result["template_key"], result)
    return result


@router.post("/prompt/templates/{template_key:path}/reset")
...
def reset_prompt_v2_template(
    ...
    try:
        ...
        raise HTTPException(400, str(e))
    audit_request(db, request, "reset_prompt_template", "prompt", result["template_key"], result)
    return result


@router.get("/prompt/flow")
...
def save_prompt_v2_flow(
    ...
    try:
        ...
        raise HTTPException(400, str(e))
    audit_request(db, request, "save_prompt_flow", "prompt_flow", "chat_flow", result)
    return result
