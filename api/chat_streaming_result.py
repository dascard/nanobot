...
async def push_stream_finalization_result(
    ...
    """只投递已完成的流式结果，不重复持久化或 settlement。"""
...
async def persist_stream_result_after_runner_done(
    ...
    *,
    ...
    try:
        ...
        def _write_success(db_for_write: Any) -> int:
            return callbacks.persist_chat_turn(
                db_for_write,
                context.persist_req,
                final_answer,
                context.guardrail_status,
        ...
        if persist_db is not None:
            pending = _write_success(persist_db)
        else:
            from core.uow import UnitOfWork
