...
import { DriverBadge, InlineNotice, PricingPills, StatePill, formatApiError, formatTime, inputClass } from './modelConsoleUi'

export function ModelCatalogPanel({ providers, presets, onChanged }) {
  ...
  const presetsByModel = useMemo(() => {
    const grouped = new Map()
    presets.forEach(preset => {
      const key = `${preset.provider_id}::${preset.model}`
      grouped.set(key, [...(grouped.get(key) || []), preset])
    })
    return grouped
  }, [presets])
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-slate-800 bg-slate-900">
      <header className="flex flex-col gap-3 border-b border-slate-800 px-4 py-3 sm:flex-row sm:items-end sm:justify-between sm:px-5">
        <div><h2 className="text-sm font-semibold text-slate-100">Model Catalog Snapshot</h2><p className="mt-1 text-[11px] text-slate-500">Catalog 是上游 `/models` 的发现快照；价格来自匹配的 Preset，stale 时保留最后一次成功结果。</p></div>
      ...
      <div className="overflow-x-auto p-4 sm:p-5">
        <table className="min-w-[980px] w-full text-left text-xs"><thead className="border-b border-slate-800 text-[10px] uppercase tracking-wide text-slate-600"><tr><th className="px-3 py-2">Model ID</th><th className="px-3 py-2">Provider</th><th className="px-3 py-2">Preset 价格</th><th className="px-3 py-2">状态</th><th className="px-3 py-2">Route 引用</th><th className="px-3 py-2">更新时间</th></tr></thead><tbody>{loading && <tr><td colSpan="6" className="px-3 py-8 text-center text-slate-600">加载中...</td></tr>}{!loading && catalog.length === 0 && <tr><td colSpan="6" className="px-3 py-8 text-center text-slate-600">目录为空；请同步支持 Model Discovery 的 Provider。</td></tr>}{catalog.map(model => {
          const provider = providers.find(item => item.id === model.provider)
          const matchedPresets = presetsByModel.get(model.id) || []
          return <tr key={model.id} className="border-b border-slate-800/70 transition-colors last:border-0 hover:bg-slate-800/30"><td className="px-3 py-2.5 font-mono text-slate-200">{model.model}</td><td className="px-3 py-2.5"><div className="flex items-center gap-2"><span className="text-slate-400">{model.provider}</span>{provider && <DriverBadge driver={provider.driver_type} />}</div></td><td className="px-3 py-2.5">{matchedPresets.length > 0 ? <div className="space-y-1.5">{matchedPresets.map(preset => <div key={preset.id}><div className="mb-1 font-mono text-[10px] text-slate-500">{preset.id}</div><div className="flex flex-wrap gap-1"><PricingPills inputCost={preset.cost_input_1m} outputCost={preset.cost_output_1m} /></div></div>)}</div> : <span className="text-slate-600">未配置</span>}</td><td className="px-3 py-2.5">{model.stale ? <StatePill ok={false}>stale</StatePill> : <StatePill ok>verified</StatePill>}</td><td className="px-3 py-2.5 text-slate-500">{(usedBy.get(model.id) || []).join(', ') || '-'}</td><td className="px-3 py-2.5 text-[10px] text-slate-600">{formatTime(model.updated_at)}</td></tr>
        })}</tbody></table>
      </div>
    </section>
  )
