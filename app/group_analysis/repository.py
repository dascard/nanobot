...
class GroupAnalysisRepository:
    ...
    def _normalize_direct_group_id(group_id: str) -> str:
        ...
        if raw.isdigit():
            return normalize_group_session_id(raw)
    ...
    def _build_group_ref(self, group_id: str) -> GroupRef | None:
        ...
        if not user:
            stream_id = normalize_group_stream_id(ngid)
    ...
    def get_group_candidates(self, group_id: str) -> list[dict]:
        ...
        if not group_id:
            return []
        return [
            {"id": raw_group_id(item["group_id"]), "name": item["name"]}
    ...
    def _find_group_candidates(self, query: str, limit: int = 10) -> list[dict]:
        ...
        # 旧表没有 chat_type 列，因此先有界读取，再通过类型化身份解析过滤。
        for row in (
            self.db.query(User)
            .order_by(User.id)
            .limit(5000)
