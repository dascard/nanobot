...
- **实施状态（2026-06-17）**：粗略路径第 ① 步已完成并验证，H29 第一刀已提取 `PromptRuntimeInput` 请求组装边界。P1-5「Prompt legacy 收口」已完成：live `fallback_v1` 发送路径已禁用，`reply-test` / `reply-eval` 默认与旧 alias 已转向 V2-only，legacy / managed 管理写入口已降级为只读迁移入口。P1-6 下一步迁移旧任务 prompt、删除冗余资产并去版本化。
