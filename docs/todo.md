...
- **剩余**：核心混合决策主线、私聊可观测闭环、P3-3A 标注审计复跑入口和 P3-3B 仓库自包含 CI / PR gate 均已完成。更多真实样本的选择、标注仲裁、候选趋势报表和定期复跑均已进入运营闭环；是否按周期报告调参仍是后续运营动作。
- **关联**：H2 已完成 admin route 异步化和 repeats 收紧；后续与路线项 8（评测体系）、路线项 5/7（响应信封与调试可观测）继续协同。
- **下一步**：路线项 8 / P4 已完成通用 `candidates → labeled` 标注闭环、Admin 契约化工作台、P4-3 per-capability 数据集扩展、P4-4 RAG baseline gate、P4-5A 统一 PR gate、P4-5B 周期性复跑与报告归档、P4-5C RAG manual 样本扩充、P4-5D memory fixture-backed positive RAG case、P4-5E knowledge fixture citation 正例、P4-5F sticker fixture sendable 正例、P4-5G group_memory fixture 正例、P4-5H RAG 过滤约束 fixture，以及真实样本运营中的 TimingGate 信号周期审计、RAG generated → manual 仲裁入口、EvalCandidate 运营规则、候选 reject / defer 仲裁状态、人工仲裁批次审计和 EvalCandidate 运营趋势报表。下一阶段继续真实样本运营动作，可优先考虑按周期报告调参，或先补周期运行 manifest 后再做跨 artifact 趋势。
