...
当前推进焦点：TimingGate proposal 运营链路已进入只读复核和运营闭环，代码迭代优先级已转回 `docs/todo.md` 的剩余质量项；P3 超大文件队列已完成当前收口，`api/admin_routes.py` 已保持 633 行并移出 >800 行清单，普通 `api/routes.py` 已通过 Chat Route Runner 拆分降至 783 行。当前处于「ruff 批量清理」阶段，F821、F841、F541、E401、E712、F402、E741、F811、E701/E702、E402、F401 与旧式 typing 泛型已完成清零；naive datetime / DTZ 已完成 eval artifact、生产 artifact / 中文展示、测试相对时间、新闻日报测试 fixture、EvalCandidate DB naive helper、Admin Session Memory Browser DB fixture、Daily Digest 测试本地时间 helper、测试 DB fixture 第八批、RAG / sticker fixture 第九批、DTZ001 收尾、EvalCandidate 合同测试 DB now helper、群体记忆测试 DB fixture、历史上下文测试 cutoff fixture 和生产侧 rolling summary / tracing DB 时间 helper 清理，全仓 DTZ 剩余 149 个。DB/ORM 当前仍是 naive 本地时间语义，后续必须按 DB 写入、运行时比较、缓存 TTL、测试夹具分批处理，不能直接全仓替换为 UTC aware。全仓仍有其他历史大文件未纳入本轮 P3 队列，如需治理应另立计划。
