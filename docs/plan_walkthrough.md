...
当前推进焦点：TimingGate proposal 运营链路已进入只读复核和运营闭环，P3 超大文件队列已完成当前收口，`api/admin_routes.py` 已保持 633 行并移出 >800 行清单，普通 `api/routes.py` 已通过 Chat Route Runner 拆分降至 783 行。`docs/todo.md` 中「ruff 批量清理」已完成：F821、F841、F541、E401、E712、F402、E741、F811、E701/E702、E402、F401、旧式 typing 泛型和 naive datetime / DTZ 均已按批次清零，tracked Python 当前通过 Ruff 默认检查。DB/ORM 仍保持 SQLite naive 本地时间语义，后续若要统一迁移 aware 时间、继续治理其他历史大文件或新增架构演进项，应另立设计和计划。

2026-06-25 Ruff DTZ 第三十批 Daily Digest、定时任务与新闻搜索收尾：本阶段处理 `core/daily_digest.py`、`creatures/nanobot/prompts/skills/schedule_task/tool.py`、`creatures/nanobot/prompts/skills/news_search/runtime_cache.py`、`search_backend.py`、`tool.py`、`legacy_report.py`、`evidence.py`、`news_daily/tool.py`、`news_daily/pipeline/digest.py`、`normalize.py`、`normalize_v2.py`、`rank.py`、`news_daily/sources/adapters.py`、`curated.py` 和 `rss.py` 中剩余 32 个 DTZ 命中。Daily Digest 查询日期、任务 prompt 时间和 `last_run_at` 继续使用 `db_now_naive()` 的 SQLite naive 本地墙钟语义；新闻搜索的缓存 key、日期抽取、展示时间、最近性判断和 RSS / HTML 日期解析保持原有业务口径，裸 `datetime.now()` 改为 `db_now_naive()`，无时区 `strptime()` 改为 `fromisoformat()`、`parsedate_to_datetime()` 或手写日期解析。验证包括目标文件 DTZ 绿灯 `All checks passed`、tracked Python Ruff 默认检查 `All checks passed`、Daily Digest / schedule / push 定向回归 `21 passed, 21 warnings in 3.18s`、新闻搜索 / news_daily 定向回归 `120 passed, 1 warning in 4.85s`、全量回归 `1806 passed, 6 skipped, 139 warnings in 128.89s`；tracked Python DTZ 已清零，`docs/todo.md` 的「ruff 批量清理」已标记完成。

2026-06-25 Ruff DTZ 第二十九批核心上下文与群分析时间清理：本阶段处理 `core/context_builder.py`、`core/context_legacy.py`、`core/expression_learner.py`、`core/group_runtime/state.py`、`core/persona_preprocess.py`、`core/semantic/scoring.py` 和 `creatures/nanobot/prompts/skills/group_analysis/repository.py` 中 11 个上下文窗口、prompt 时间展示、群运行时 pending payload、画像状态机和群分析 SQL cutoff 命中，不触碰 `core/daily_digest.py`、新闻搜索日期解析或 schedule task 工具。ChatLog / Persona / 语义 recency 的运行时比较统一复用 `db_now_naive()`，继续对齐 SQLite naive 本地墙钟语义；群 pending 的 epoch 秒转换改为 aware UTC 解析后转本地 naive，保留本地墙钟展示。验证包括本批目标文件 DTZ 绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `94 passed, 3 skipped, 1 warning in 5.61s`；全仓 DTZ 统计从 43 降至 32。

2026-06-25 Ruff DTZ 第二十八批 Admin/API 与 session memory 日期边界清理：本阶段处理 `api/admin/reply_routes.py`、`api/admin/runtime_routes.py`、`api/memory_routes.py` 和 `app/session_memory/retrieval_service.py` 中 11 个 Admin/API cutoff、DB 更新时间、默认日期和日期边界解析命中，不触碰 `core/daily_digest.py`、新闻搜索日期解析或 prompt 展示时间。回复评测 case 更新时间、真实流量窗口、runtime overview / group 统计窗口和 `since_last_reply` 均复用 `db_now_naive()`，记忆摘要默认目标日期按本地 naive 自然日取昨天，session summary 日期过滤改用 `datetime.fromisoformat()` 保持 SQLite naive 日期边界；回复评测默认运行名称保留本地展示时间并改为 `time.strftime()`，nullable `created_at` 排序哨兵改为空值安全 key。验证包括本批目标文件 DTZ 绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `65 passed, 21 warnings in 6.18s`；全仓 DTZ 统计从 54 降至 43。

2026-06-24 Ruff DTZ 第二十七批 core 旧记忆/表情/画像 DB 时间清理：本阶段只处理 `core/ai_daily_ingest.py`、`core/expression_memory.py`、`core/group_memory.py`、`core/legacy_adapter.py` 和 `core/sticker_memory.py` 中 14 个 ORM 写入时间命中，不触碰 ChatLog cutoff、prompt 展示时间、semantic recency 评分或新闻日期解析。AI Daily 入库、Expression / Jargon 更新、GroupMemory upsert、旧 KT adapter 画像 / SystemPrompt 回写、StickerMemory 注册 / 使用 / 描述时间均复用 `db_now_naive()`，其中同一次创建或批量更新复用单个 `now`，继续对齐 SQLite ORM naive 本地墙钟语义。验证包括目标文件 DTZ 绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `41 passed, 1 warning in 4.30s` 与 `34 passed, 1 warning in 3.68s`；全仓 DTZ 统计从 68 降至 54。

2026-06-24 Ruff DTZ 第二十六批群体记忆 RAG 缓存 TTL 清理：本阶段只处理 `app/group_memory/injection_service.py` 中 3 个内存缓存 TTL 命中，不触碰 DB 写入时间和群体记忆检索评分。`GROUP_MEMORY_RAG_CACHE` 的过期戳从 `datetime` 改为 `float` 单调时间，命中判断和写入统一使用 `time.monotonic()`，避免 wall-clock 调整影响短 TTL 缓存；新增缓存 TTL 行为测试，覆盖首次写入、TTL 内命中和过期后重新查询。验证包括目标文件 DTZ 绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `7 passed, 1 warning in 1.15s`；全仓 DTZ 统计从 71 降至 68。

2026-06-24 Ruff DTZ 第二十五批 app 层记忆/画像 DB naive 时间清理：本阶段处理 `app/group_ingress/helpers.py`、`app/group_memory/retrieval_service.py`、`app/group_memory/injection_service.py`、`app/persona/injection_service.py`、`app/persona/retrieval_service.py` 和 `app/session_memory/llm_summarizer.py` 中 7 个 ORM 写入 / ORM 历史时间比较命中，不触碰 `GROUP_MEMORY_RAG_CACHE` 的 3 个缓存 TTL 命中，后者应单独改为 `time.monotonic()`。重复群回复窗口、群体记忆 / 画像 recency、注入记录时间和 LLM rolling summary 写入时间均复用 `db_now_naive()`，LLM summary 的 `created_at` / `updated_at` 使用同一个 `now`，继续对齐 SQLite ORM naive 本地墙钟语义。验证包括目标文件常规 Ruff / `compileall` 通过、非缓存 TTL 目标文件 DTZ 绿灯 `All checks passed`、定向回归 `104 passed, 3 skipped, 21 warnings in 7.04s`；全仓 DTZ 统计从 78 降至 71。

2026-06-24 Ruff DTZ 第二十四批 Admin/API DB naive 时间清理：本阶段只处理 `api/admin/persona_routes.py`、`api/history_log_routes.py` 和 `api/task_routes.py` 中 4 个低风险 DB 时间命中，不触碰回复评测展示名称、runtime overview 响应格式或中文自然日默认值。画像提取窗口 cutoff、Persona.updated_at、mark-clear 的 `history_clear_at` / ConversationTurn 删除 cutoff、ScheduledTask.last_run_at 均复用 `db_now_naive()`，其中 mark-clear 保持单个 `now` 同时写标记和删除旧上下文，继续对齐 SQLite ORM naive 本地墙钟语义。验证包括目标文件 DTZ 绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `24 passed, 21 warnings in 3.60s`；全仓 DTZ 统计从 82 降至 78。

2026-06-24 Ruff DTZ 第二十三批 DB 管理辅助时间清理：本阶段只处理 `core/schema_migrations.py`、`core/runtime_tool_service.py` 和 `core/knowledge_library.py` 中 4 个低风险命中，不触碰调度状态机、检索窗口、新闻日期解析或 prompt 输出格式。migration `applied_at`、RuntimeToolDecision 旧记录清理 cutoff 和 KnowledgeDocument / KnowledgeChunk 写入时间均复用 `db_now_naive()`，保留 SQLite ORM naive 本地墙钟语义；migration 备份文件名改为 `time.strftime("%Y%m%d_%H%M%S")`，保持本地墙钟文件名语义。验证包括目标文件 DTZ 绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `46 passed, 139 warnings in 8.97s`；全仓 DTZ 统计从 86 降至 82。

2026-06-24 Ruff DTZ 第二十二批本地测试脚本时间戳清理：本阶段只处理 `tests/local_test.py` 中 1 个本地调试 artifact 文件名时间戳 `datetime.now()` 命中，不触碰 chat / push 请求逻辑、监听器或输出内容。该文件已有 `time.strftime("%H%M%S")` 用于 chat 输出文件名，本批把 push raw / json 输出文件名改为同风格的 `time.strftime("%Y%m%d_%H%M%S")`，并移除不再使用的 `datetime` 导入，保持本地墙钟文件名语义。验证包括目标文件 DTZ 绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过；全仓 DTZ 统计从 87 降至 86。

2026-06-24 Ruff DTZ 第二十一批 DB-backed 测试 fixture 清理：本阶段只处理 `tests/test_persona_injection_service.py`、`tests/test_prompt_trace_admin.py`、`tests/test_rag_debug.py` 和 `tests/test_reply_admin.py` 中 4 个测试侧 `datetime.now()` 命中，不触碰 persona injection、trace admin、RAG debug 或 reply eval 生产逻辑。测试侧新增 `_local_now()`，集中说明 PersonaFact、RunTracer / trace DB、GroupMemory 和 ReplyContractCheckLog fixture 保持 naive 本地墙钟时间语义；reply traffic 统计继续使用单个 `now` 基准派生最近 24 小时和 3 天前日志，保留窗口过滤断言。验证包括目标文件 DTZ 绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `36 passed, 21 warnings in 8.88s`；全仓 DTZ 统计从 91 降至 87。

2026-06-24 Ruff DTZ 第二十批群分析测试 fixture 清理：本阶段只处理 `tests/test_group_analysis_tool.py` 中 3 个测试侧 `datetime.now()` 命中，不触碰 group analysis tool、analyzer、preprocess 或 deterministic fallback 生产逻辑。测试侧新增 `_local_now()`，集中说明 ChatLog ORM fixture 与 RawChatLog dataclass fixture 保持 naive 本地墙钟时间语义；三处群分析用例继续捕获单个 `now` 基准后派生相对时间，保留最近 2 小时窗口、artifact 过滤和 LLM 失败降级测试原意。验证包括目标文件 DTZ 绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `21 passed, 1 warning in 1.63s`；全仓 DTZ 统计从 94 降至 91。

2026-06-24 Ruff DTZ 第十九批 RAG recency 测试 fixture 清理：本阶段只处理 `tests/test_knowledge_rag.py`、`tests/test_memory_query_rag.py` 和 `tests/test_sticker_rag.py` 中 3 个测试侧 `datetime.now()` 命中，不触碰 knowledge / memory / sticker RAG 生产检索、reranker 或 recency 评分逻辑。测试侧新增 `_local_now()`，集中说明 KnowledgeDocument、SemanticIndexItem 和 StickerMemory 等 SQLite ORM fixture 保持 naive 本地墙钟时间语义；各 recency 测试继续用单个 `now` 基准派生旧/新时间，保持相对排序断言不变。验证包括目标文件 DTZ 绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `45 passed, 1 warning in 5.55s`；全仓 DTZ 统计从 97 降至 94。

2026-06-24 Ruff DTZ 第十八批画像预处理测试 fixture 清理：本阶段只处理 `tests/test_persona_preprocess.py` 中 8 个测试侧 `datetime.now()` 命中，不触碰 `PersonaStateMachine`、画像衰减算法、候选处理或摘要构建生产逻辑。测试侧新增 `_local_now()`，集中说明 PersonaFact 测试 DB fixture 保持 naive 本地墙钟时间语义；衰减测试捕获单个 `now` 后派生旧/新时间，并将同一基准传入 `_apply_decay(now)`，避免测试内当前时间漂移。验证包括目标文件 DTZ 红灯 8 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `33 passed, 3 skipped, 1 warning in 1.38s`；全仓 DTZ 统计从 105 降至 97。

2026-06-24 Ruff DTZ 第十七批 Admin API 测试 fixture 清理：本阶段只处理 `tests/test_admin_api.py` 中 12 个测试侧 `datetime.now()` 命中，不触碰 Admin API、群记忆、画像治理、TimingGate 或工具目标列表生产逻辑。测试侧新增 `_local_now()`，集中说明 Admin API 测试 DB fixture 保持 naive 本地墙钟时间语义；ChatLog、ConversationTurn、GroupMemory 和 PersonaFact 等 ORM fixture 继续使用单个 `now` 基准，保留 overview 近 1 小时窗口、recent decision 排序、画像事实时间和工具目标过滤测试原意。验证包括目标文件 DTZ 红灯 12 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `30 passed, 1 warning in 4.89s`；全仓 DTZ 统计从 117 降至 105。

2026-06-24 Ruff DTZ 第十六批任务相关测试 fixture 清理：本阶段只处理 `tests/test_session_memory.py` 和 `tests/test_semantic_index_worker.py` 中 10 个测试侧 `datetime.now()` 命中，不触碰 session memory、semantic index worker、job recover 或 rolling summary 生产逻辑。测试侧新增 `_local_now()`，集中说明 SQLite ORM `DateTime` fixture 保持 naive 本地墙钟时间语义；raw window、history clear、群上下文 rollup、session summary worker stale job 和 semantic index worker transaction / recover fixture 均复用同一 helper 或单个 `now` 基准。验证包括目标文件 DTZ 红灯 10 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `43 passed, 21 warnings in 3.17s`；全仓 DTZ 统计从 127 降至 117。

2026-06-24 Ruff DTZ 第十五批 job 队列 DB 时间 helper 清理：本阶段处理 `app/session_memory/jobs.py`、`core/semantic/jobs.py` 和 `workers/semantic_index_worker.py` 中 22 个 job 状态机 / worker DB 写入时间命中，不修改 `SessionSummaryJob` 或 `SemanticIndexJob` 的 retry 状态语义，也不触碰 ORM Column default。session summary job 对外部注入的 `now` 使用 `to_db_naive()` 归一化，claim / recover / failed retry 继续按同一函数单次 `now` 基准比较和写入；semantic index job 和删除 worker 写入使用 `db_now_naive()`，同一次创建、领取、恢复、完成或失败迁移复用同一时间。验证包括目标文件 DTZ 红灯 22 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `43 passed, 21 warnings in 3.10s`；全仓 DTZ 统计从 149 降至 127。

2026-06-24 Ruff DTZ 第十四批生产侧 rolling summary / tracing DB 时间 helper 清理：本阶段只处理 `app/session_memory/rolling_summary.py` 和 `core/tracing.py` 中 12 个生产侧 ORM 写入时间命中，不触碰 `SessionSummaryJob` retry 状态机、语义索引 job 状态机或运行时窗口比较逻辑。新增 `core/time_utils.py`，提供 `db_now_naive()` 和 `to_db_naive()`，以 aware 当前时间转回本地 naive 后写入现有 SQLite ORM `DateTime` 字段；`RunTracer` / `ToolTracer` 的外部 `finished_at` 入参保持兼容，若传入 aware datetime 则先归一化为本地 naive。验证包括目标文件 DTZ 红灯 12 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `70 passed, 21 warnings in 6.16s`；全仓 DTZ 统计从 161 降至 149。

2026-06-24 Ruff DTZ 第十三批历史上下文测试 cutoff fixture 清理：本阶段只处理 `tests/test_history.py` 中 11 个测试侧 `datetime.now()` 命中，不触碰 `_build_session_memory()`、`build_chat_context()`、history clear 或 rolling summary 生产逻辑。测试侧新增 `_local_now()`，集中说明 ChatLog / ConversationTurn / RollingSessionSummary / User cutoff fixture 保持 SQLite naive 本地墙钟时间语义；私聊/群聊 raw window、rolling summary 裁剪、ChatLog 群现场和 mark-clear 测试均捕获单个 `now` 后派生旧/新时间，避免测试内多次当前时间造成边界漂移。验证包括目标文件 DTZ 红灯 11 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `21 passed, 1 warning in 2.92s`；全仓 DTZ 统计从 172 降至 161。

2026-06-24 Ruff DTZ 第十二批群体记忆测试 DB fixture 清理：本阶段只处理 `tests/test_group_memory_injection.py`、`tests/test_group_memory_rag.py` 和 `tests/test_group_memory_extraction_service.py` 中 13 个测试侧 `datetime.now()` 命中，不触碰生产群体记忆注入、检索、提取服务或 RAG scoring 逻辑。测试侧新增 `_local_now()`，集中说明群体记忆 ORM fixture、窗口查询 fixture 和相对新旧程度 fixture 保持 SQLite naive 本地墙钟时间语义；相对时间测试捕获单个 `now` 基准后派生旧/新时间，避免测试内微小漂移。验证包括目标文件 DTZ 红灯 13 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `15 passed, 1 warning in 2.54s`；全仓 DTZ 统计从 185 降至 172。

2026-06-24 Ruff DTZ 第十一批 EvalCandidate 合同测试 DB now helper 清理：本阶段只处理 `tests/test_eval_candidate_contract.py` 中 3 个测试侧 `datetime.now()` 命中，不触碰 `core/eval_sampling/store.py` 的生产趋势聚合逻辑或 Admin API 合同。测试侧新增 `_db_now()`，集中说明 EvalCandidate ORM fixture 和本地日期分桶测试保持 SQLite naive 本地墙钟时间语义；趋势测试和只读 API fixture 均改为 helper。验证包括目标文件 DTZ 红灯 3 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `34 passed, 21 warnings in 5.43s`；全仓 DTZ 统计从 188 降至 185。

2026-06-24 Ruff DTZ 第十批 DTZ001 收尾：本阶段只处理 `creatures/nanobot/prompts/skills/news_search/runtime_cache.py` 与 `creatures/nanobot/prompts/skills/news_search/news_daily/sources/rss.py` 中剩余 2 个 DTZ001，不触碰 `_extract_date()` 当前本地日期语义、RSS `_is_recent()` naive 比较或其他 DTZ005 / DTZ007 生产边界。`_coerce_date()` 从 `datetime(...).strftime("%Y-%m-%d")` 改为 `date(...).isoformat()`，只保留原本的日期有效性校验和字符串输出；RSS `published_parsed` 从构造 naive datetime 改为取年月日构造 `date` 并输出 ISO 日期。验证包括目标文件 DTZ001 红灯 2 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `35 passed, 1 warning in 2.84s`；全仓 DTZ 统计从 190 降至 188，DTZ001 已清零。

2026-06-24 Ruff DTZ 第九批 RAG / Sticker fixture 清理：本阶段只处理 `evals/rag_benchmark/fixtures.py` 与 `tests/test_sticker_rag.py` 中 4 个固定 ORM 时间，不触碰生产缓存、RSS 日期解析或 runtime cache。测试 / eval fixture 侧新增 `_db_time()`，集中说明这些写入 SQLite ORM `DateTime` 的 fixture 保持 naive 本地墙钟时间语义，并用单点 `noqa` 取代重复 DTZ001 命中。验证包括目标文件 DTZ 红灯 4 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `47 passed, 21 warnings in 9.34s`、全量回归 `1805 passed, 6 skipped, 139 warnings in 121.59s`；全仓 DTZ 统计从 194 降至 190。

2026-06-24 Ruff DTZ 第八批测试 DB fixture 清理：本阶段只处理 `tests/test_rag_benchmark_admin.py`、`tests/test_memory_digest.py` 和 `tests/test_memory_digest_builder_quality.py` 中 6 个固定 ORM 时间 fixture，不触碰生产代码、评测 fixture 数据层或 MemoryDigest 查询契约。测试侧新增 `_db_time()`，集中说明 SQLite ORM `DateTime` fixture 保持 naive 本地墙钟时间语义，并用单点 `noqa` 取代重复 DTZ001 命中。验证包括目标文件 DTZ 红灯 6 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `52 passed, 21 warnings in 9.00s`、全量回归 `1805 passed, 6 skipped, 139 warnings in 122.37s`；全仓 DTZ 统计从 200 降至 194，剩余继续集中在 Admin/API 测试夹具、history/session memory、session/semantic job、tracing、生产新闻日期解析与业务模块时间窗口。

2026-06-24 Ruff DTZ 第七批 Daily Digest 测试本地时间 helper 清理：本阶段只处理 `tests/test_daily_digest.py` 中日报测试的本地时间 fixture，不触碰 `core/daily_digest.py` 生产调度、查询窗口或 ScheduledTask 写入逻辑。测试侧新增 `_local_time()` / `_local_now()`，集中说明日报 DB fixture 和定时任务 prompt 保持生产侧 naive 本地墙钟时间语义；固定 ChatLog 时间、`_build_scheduled_task_query()` 北京时间入参和 last_run_at 近实时断言均改为 helper。验证包括目标文件 DTZ 红灯 7 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `11 passed, 1 warning in 1.05s`、全量回归 `1805 passed, 6 skipped, 139 warnings in 122.39s`；全仓 DTZ 统计从 207 降至 200，剩余继续集中在 Admin/API 测试夹具、history/session memory、session/semantic job、tracing、生产新闻日期解析与业务模块时间窗口。

2026-06-24 Ruff DTZ 第六批 Admin Session Memory Browser DB fixture 清理：本阶段只处理 `tests/test_admin_session_memory_browser.py` 中 10 个固定 DB 时间 fixture，不触碰 Admin Session Memory Browser 生产查询、序列化和 HTTP 契约。测试侧新增 `_db_time()`，集中说明 SQLite ORM `DateTime` fixture 仍按 naive 本地墙钟时间写入，并把 10 个重复的 `datetime(...)` 调用改为 helper，避免把 DB fixture 误改为 UTC aware 后改变现有 naive 语义。验证包括目标文件 DTZ 红灯 10 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `10 passed, 21 warnings in 4.07s`、全量回归 `1805 passed, 6 skipped, 139 warnings in 122.01s`；全仓 DTZ 统计从 217 降至 207，剩余继续集中在 Admin/API 测试夹具、history/session memory、session/semantic job、tracing、生产新闻日期解析与业务模块时间窗口。

2026-06-24 Ruff DTZ 第五批 EvalCandidate DB naive helper 清理：本阶段只处理 `core/eval_sampling/store.py` 的 EvalCandidate / EvalRun / cursor DB 时间戳和候选趋势本地日期分桶基准，不改变状态机、JSON 合同、Admin API 或 CLI 输出。新增 `_db_now_naive()`，通过 aware UTC 当前时间转为本地 naive 墙钟时间，保留当前 SQLite / SQLAlchemy `DateTime` 的 naive 本地语义，避免把 UTC aware 写入 ORM 后被 SQLite 剥掉 offset。验证包括目标文件 DTZ 红灯 10 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、EvalCandidate / candidate CLI 定向回归 `43 passed, 21 warnings in 6.07s`、全量回归 `1805 passed, 6 skipped, 139 warnings in 121.16s`；全仓 DTZ 统计从 227 降至 217，剩余继续集中在 Admin/API 测试夹具、history/session memory、session/semantic job、tracing、生产新闻日期解析与业务模块时间窗口。

2026-06-24 Ruff DTZ 第四批新闻日报测试 fixture 清理：本阶段只处理 `tests/test_news_daily_pipeline.py`，不触碰新闻生产代码和 `parse_date()` 当前返回 naive datetime 的契约。测试侧新增固定 aware UTC 基准 `FIXED_NOW_UTC`，把 freshness、cluster、diversify、report 和 digest fixture 中的动态 `datetime.now()` 改为同一 aware UTC 测试时间；4 个验证 `parse_date()` naive 返回值的断言保留 naive expected datetime，并加定点 `noqa: DTZ001` 说明契约。验证包括目标文件 DTZ 红灯 44 个命中、绿灯 `All checks passed`、目标文件常规 Ruff / `compileall` 通过、定向回归 `78 passed, 1 warning in 1.47s`、全量回归 `1805 passed, 6 skipped, 139 warnings in 123.27s`；全仓 DTZ 统计从 271 降至 227，剩余继续集中在 DB 写入/比较、session/semantic job、tracing、生产新闻日期解析与业务模块时间窗口。

2026-06-24 Ruff DTZ 第三批测试相对时间清理：本阶段只处理不写入 ORM、不参与数据库时间比较的测试侧相对时间用例，覆盖 `tests/test_ai_daily_tool_and_sources.py`、`tests/test_tools_package.py`、`tests/test_group_analysis_local_rag.py` 和 `tests/test_semantic_scoring.py`。新闻新鲜度测试使用 aware UTC 生成日期字符串，群分析 local RAG 临时消息时间改为 aware UTC；`test_recency_score_accepts_iso_z_timestamp_with_naive_reference` 明确验证 naive reference 兼容，保留 naive datetime 并加定点 `noqa: DTZ001`，避免把行为契约改没。验证包括候选测试文件 DTZ 红灯 10 个命中、绿灯 `All checks passed`、候选文件常规 Ruff / `compileall` 通过、定向回归 `57 passed, 1 warning in 3.51s`、全量回归 `1805 passed, 6 skipped, 139 warnings in 122.35s`；全仓 DTZ 统计从 281 降至 271，剩余继续集中在 DB 写入/比较、news daily 解析测试夹具、session/semantic job、tracing 和业务模块时间窗口。

2026-06-24 Ruff DTZ 第二批生产 artifact / 中文展示清理：本阶段继续避开 ORM `DateTime` 写入、历史窗口比较、job retry/lock 和业务 recency 计算，只处理 Admin / RAG benchmark artifact metadata、群记忆提取 metadata、生成图片 / Prompt 文件 mtime 展示，以及新闻日报 / 群分析中文用户展示时间。`api/admin/system_routes.py`、`api/admin/model_routes.py`、`api/admin/rag_benchmark_routes.py`、`app/group_memory/extraction_service.py` 使用 aware UTC 记录机器可读 metadata；`core/generated_images.py` 与 `core/prompts/manager.py` 从 epoch / mtime 转本地 aware ISO，保留原本本地墙钟展示语义；`group_analysis/render.py`、`news_search/render.py`、`news_daily/cache.py`、`news_daily/schema.py` 和 `news_plugin/schema.py` 使用 `Asia/Shanghai`，避免中文日报在 00:00-07:59 落到 UTC 前一日。验证包括候选文件 DTZ 红灯 22 个命中、绿灯 `All checks passed`、候选文件常规 Ruff / `compileall` / 上海时区 smoke 通过、定向回归 `99 passed, 21 warnings in 11.06s`、全量回归 `1805 passed, 6 skipped, 139 warnings in 121.31s`；全仓 DTZ 统计从 303 降至 281，剩余继续集中在 DB 写入/比较、news daily 解析测试夹具、session/semantic job、tracing 和业务模块时间窗口。
...
2026-06-24 Ruff F401 第五批 Admin facade 收尾：本阶段保守处理 `api/admin_routes.py` 中 235 个 Admin legacy facade 兼容导出，不删除拆分子路由符号，不使用 `ruff --fix`，而是将 db browser、group memory、log、model、reply、eval、chat config、sticker、runtime、trace 和 tool 模块导入的旧路径兼容符号改为冗余别名 re-export。该方案保留 `api.admin_routes.<symbol>` 真实模块属性，继续支持旧显式导入、测试 monkeypatch、`NANOBOT_ADMIN_TOKEN` 兼容鉴权、`TIMING_TUNING_PROPOSAL_REPORT` 路径 monkeypatch 和 log viewer 对 `api.admin_routes.__file__` 的兼容读取，同时避免新增超大 `__all__` 块导致文件重新超过 800 行。tracked Python F401 已清零；验证包括 `api/admin_routes.py` 与 `api/admin/*.py` 的 `ruff check --select F401`、tracked Python 全仓 `ruff check`、`api/admin_routes.py` 的 `compileall`、`git diff --check`、Admin facade 定向回归 `74 passed, 21 warnings in 13.33s`、鉴权与 proposal admin 定向回归 `12 passed, 21 warnings in 3.62s` 和全量回归 `1805 passed, 6 skipped, 139 warnings in 122.45s`。

2026-06-24 Ruff F401 第四批普通 API facade 清理：本阶段删除 `api/routes.py` 中 13 个无兼容证据的真未用导入，包括 `os`、`Response`、旧 OpenAI / New API 配置常量、`check_message_moderation_db` 和 `get_timing_gate`；保留 `NANOBOT_API_TOKEN` 作为 `api.common_auth` 旧 monkeypatch 入口，保留 `_sanitize_prompt_text` 与 `_build_session_memory` 作为历史 helper 导入路径，并将 evolution、history、memory、model、task、sticker media、agent step、group message 和 group utility 的旧路径导出改为冗余别名形式。tracked Python Ruff 统计从 F401 ×303 降至 F401 ×235，剩余集中在 `api/admin_routes.py` 的 Admin legacy facade。验证包括 `api/routes.py` 的 `ruff check --select F401`、相关文件 `compileall`、`git diff --check`、普通 API facade 定向回归 `109 passed, 21 warnings in 12.96s` 和全量回归 `1805 passed, 6 skipped, 139 warnings in 123.07s`。

2026-06-24 Ruff F401 第三批 Admin Models 清理：本阶段删除 `api/admin/model_routes.py` 中 6 个函数内真未用导入，包括 `models_status()` 内未引用的 `build_model_catalog`，以及 `_resolve_route_value()` 内未引用的 `LLM_MODEL_REPLY`、`LLM_MODEL_FAST`、`LLM_MODEL_SMART`、`CLASSIFIER_API_URL` 和 `IMAGE_SUMMARY_API_URL`；这些值的实际来源仍是 `_STAGE_META` 与 `settings.get(key)`，不承担 legacy facade 导出。tracked Python Ruff 统计从 F401 ×309 降至 F401 ×303，剩余集中在 `api/admin_routes.py` 和 `api/routes.py`。验证包括本阶段文件 `ruff check --select F401`、相关文件 `compileall`、`git diff --check`、模型路由定向回归 `23 passed, 21 warnings in 2.32s` 和全量回归 `1805 passed, 6 skipped, 139 warnings in 124.58s`。

2026-06-24 Ruff F401 第二批非大型 facade 清理：本阶段删除测试侧真未用 import，覆盖 `tests/conftest.py` 和 `tests/test_prompt_trace_admin.py`；删除 `news_search/evidence.py`、`core/legacy_adapter.py` 与 `core/persona_preprocess.py` 中确认无兼容意义的未用 import；对 `core/group_runtime/runtime.py`、`news_search/tool.py` 和 `core/persona_preprocess.py` 中拆分后仍承担旧导入路径兼容的符号改为显式 re-export，避免误删旧路径、测试 monkeypatch 或外部导入契约。tracked Python Ruff 统计从 F401 ×362 降至 F401 ×309，剩余主要集中在 API facade：`api/admin_routes.py`、`api/routes.py` 和 `api/admin/model_routes.py`。验证包括本阶段文件 `ruff check --select F401`、相关文件 `compileall`、`git diff --check`、定向回归 `197 passed, 3 skipped, 21 warnings in 14.87s` 和全量回归 `1805 passed, 6 skipped, 139 warnings in 122.94s`。

2026-06-24 Ruff F401 第一批低风险清理：本阶段使用 Ruff 安全 fix 清理 80 处低风险未用 import，覆盖 session memory、group ingress helper、memory digest、New API / classifier client、context / evolution / generated images、Prompt V2 loader、news daily pipeline、group analysis、eval runner、脚本和相邻测试文件。刻意排除 `api/admin_routes.py`、`api/routes.py`、`core/group_runtime/runtime.py` 等 facade / re-export 兼容大头，避免把兼容导出当作普通未用 import 删除；对 `context_builder`、`section_renderer`、`template_loader`、`group_ingress.helpers` 和 `tool_schema_preview` 中被外部导入或测试 monkeypatch 的兼容符号改为显式 re-export。tracked Python Ruff 统计从 F401 ×442 降至 F401 ×362。验证包括本阶段文件 `ruff check --select F401`、相关文件 `compileall`、`git diff --check`、失败相关回归 `90 passed, 21 warnings`、定向回归 `451 passed, 21 warnings` 和全量回归 `1805 passed, 6 skipped, 139 warnings in 120.17s`。

2026-06-24 Ruff F841 小批清理：本阶段删除无副作用死赋值，保留 `LogAnalystAgent.run()` 的既有 await 调用副作用；测试侧补强 KT 工具循环、群记忆候选和 news daily render guard 断言。`_report_to_digest()` 现在会把 top story 计入后续 highlight 的实体 / 域名去重，避免 top story 与 highlights 重复展示同实体事件。验证包括 tracked Python 文件 `ruff check --select F841`、相关文件 `compileall`、单个 news 红绿用例、定向回归 `174 passed, 21 warnings` 和全量回归 `1805 passed, 6 skipped, 139 warnings in 119.29s`。

2026-06-24 Ruff F541 小批清理：本阶段使用 ruff 安全 fix 移除 13 处无占位符 f-string 前缀，覆盖 Admin sticker、legacy adapter、news render/evidence、eval runner/scorer、Bridge 日志和 admin 测试 URL。验证包括 tracked Python 文件 `ruff check --select F541`、相关文件 `compileall`、定向回归 `253 passed, 21 warnings` 和全量回归 `1805 passed, 6 skipped, 139 warnings in 124.52s`。

2026-06-24 Ruff 小类别清理：本阶段清零 E401/E712/F402/E741 共 12 个命中，覆盖局部 import 拆行、歧义变量命名、dataclass `field` 遮蔽和测试中的布尔比较。验证包括 tracked Python 文件 `ruff check --select E401,E712,F402,E741`、相关文件 `compileall`、定向回归 `5 passed, 21 warnings`、相邻回归 `96 passed, 1 warning` 和全量回归 `1805 passed, 6 skipped, 139 warnings in 123.75s`。

2026-06-24 Ruff F811 小批清理：本阶段删除 `api/admin/model_routes.py` 与 `tests/test_daily_digest.py` 中重复的局部 `asyncio` import，保留既有顶层 import 和局部 `time` / `threading` 风格。验证包括 tracked Python 文件 `ruff check --select F811`、相关文件 `compileall`、定向回归 `9 passed, 1 warning` 和全量回归 `1805 passed, 6 skipped, 139 warnings in 122.63s`。

2026-06-24 Ruff E701/E702 小批清理：本阶段机械拆分一行多语句，覆盖 Admin chat config / model / sticker、model registry、New API 客户端、legacy adapter、sticker preview、news daily pipeline、news tool 和本地测试脚本；保持原执行顺序、异常覆盖范围和分支归属不变。验证包括 tracked Python 文件 `ruff check --select E701,E702`、相关文件 `compileall`、`git diff --check`、相邻回归 `116 passed, 21 warnings` 和全量回归 `1805 passed, 6 skipped, 139 warnings in 121.54s`。

2026-06-24 Ruff E402 小批清理：本阶段清零模块级 import 位置告警，覆盖 Admin legacy facade、普通 API context builder re-export、GroupRuntime future/docstring 顺序、Persona CUDA warning 初始化边界、image summary tool、news daily pipeline 测试、原生 bridge smoke 脚本和测试 conftest 初始化顺序。保留 `tests/conftest.py` 的测试环境变量先于项目模块导入，并对必要的延后 import 使用局部 `# noqa: E402`；Admin facade 通过移动 `logger/router` 赋值清理 E402，不使用整文件豁免且不改变 `router.include_router()` 顺序。验证包括 tracked Python 文件 `ruff check --select E402`、相关文件 `compileall`、定向回归 `235 passed, 3 skipped, 21 warnings` 和全量回归 `1805 passed, 6 skipped, 139 warnings in 124.36s`。
...
## 2026-06-22 普通 API Chat Runtime Facade 拆分

状态：设计、计划、红灯测试、新模块拆分、父模块接入、Prompt Runtime 核查、验证和
实现阶段提交均已完成。本阶段继续拆普通 `api/routes.py`，新增
`api/chat_runtime_facade.py`，迁移 Bridge / Prompt Runtime 输入组装边界和非流式
Bridge 调用小包装。`api.routes` 继续保留 `/chat` 路由本体、`get_bridge` patch point、
私聊缓冲、guardrail 预跑、streaming finalizer、聊天落库、response envelope、
`CHAT_STREAM_QUEUE_MAXSIZE` 和 `/health`。本阶段职责边界继续收敛；因父模块接入代码和
import 增量，`api/routes.py` 当前为 1470 行，`api/chat_runtime_facade.py` 为 156 行。

设计文档：
`docs/superpowers/specs/2026-06-22-api-chat-runtime-facade-split-design.md`。

实现计划：
`.Codex/plans/api-chat-runtime-facade-split.md`。

阶段提交：

- 设计提交：`c162ae3 docs(普通API): 设计聊天运行时门面拆分`。
- 计划提交：`bad938a docs(计划): 记录聊天运行时门面计划`。
- 红灯测试提交：`918c311 test(普通API): 锁定聊天运行时门面契约`。
- 新模块提交：`30beb56 refactor(普通API): 增加聊天运行时门面`。
- 父模块接入提交：`3854c83 refactor(普通API): 接入聊天运行时门面`。
- Prompt Runtime 核查提交：`98c719a docs(计划): 记录运行时门面模板核查`。
- 文档收口提交：随本次 `docs(计划): 收口聊天运行时门面拆分` 完成。

计划列表：

- [x] 汇总 `/chat` 剩余边界审计结论并确定 Chat Runtime Facade 边界。
- [x] 写入设计文档并提交。
- [x] 写入实现计划并提交。
- [x] 补普通 API chat runtime facade split 红灯测试并提交。
- [x] 新增 `api/chat_runtime_facade.py`，把父模块 runtime payload 组装改为调用新模块，
  保持 `get_bridge()` patch point、`bridge_meta` 字段、普通 / injection prompt 字符串、
  `stream` metadata 和非流式 `bridge.handle_message(..., stream=False)` 契约，并提交。
- [x] 核查 Prompt Runtime 模板，确认无需修改默认模板、运行时模板、
  `core/prompt_v2/variables.py` 或 `core/prompt_v2/template_registry.py`。
- [x] 更新 `docs/todo.md`、本 walkthrough 和计划执行记录，完成最终验证后提交文档收口。

Prompt Runtime 核查结论：

- `bridge_meta` 的 `persona_text`、`raw_query`、`history_header`、`history_messages`、
  `effort_constraint`、`runtime_preset`、`platform`、`chat_type` 和 `stream` 字段名与语义未变。
- 普通 `<user_input>` 包裹和 guardrail injection 安全提示未变。
- `prompts.v2.default/chat` 与 `data/prompts_v2/chat` 内容一致，模板未引用过时变量。
- 本阶段无需修改默认模板、运行时模板、`variables.py` 或 `template_registry.py`。

验证记录：

- 红灯：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_runtime_facade_split.py -v`
  -> `6 failed, 2 passed`；失败原因为 `api/chat_runtime_facade.py` 尚不存在。
- 相邻 split 扫描红灯：
...
- 普通 API split 相邻回归：
  `python -B -m pytest -p no:cacheprovider tests/test_api_history_log_routes_split.py tests/test_api_agent_step_routes_split.py tests/test_api_group_message_routes_split.py tests/test_api_sticker_media_routes_split.py -v`
  -> `45 passed, 21 warnings`。
- 父模块接入回归：先暴露并修复 `NameError: name 'platform' is not defined`；修复后
  runtime facade split `8 passed`，`/chat` 与 streaming 回归 `90 passed`，
  聊天拆分与 asyncio 策略回归 `47 passed`。
- Prompt Runtime 核查：`diff -ru prompts.v2.default/chat data/prompts_v2/chat` 无输出；
  变量扫描只命中 `identity_context.md` 中已注册的 identity 变量。
- 行数检查：`api/routes.py` 1470 行，`api/chat_runtime_facade.py` 156 行，
  `tests/test_api_chat_runtime_facade_split.py` 269 行。
- 文档收口复验：红旗词扫描无输出，`git diff --check` 无输出。
- 全量：`python -B -m pytest -p no:cacheprovider tests/ -v` ->
...
- 不拆 `/chat` 路由本体。
- 不拆 `/health`。
- 不迁移私聊缓冲、streaming runner / finalizer、聊天落库、response envelope、
  `get_bridge`、`get_guardrail` 或 `CHAT_STREAM_QUEUE_MAXSIZE`。
- 保持 `api.routes.get_bridge()` 作为 Bridge 实例 patch point。
- 不改变 Prompt Runtime 模板、`enriched_query`、conversation 结构、工具输出契约或
  message envelope。
- 不新增 `asyncio.run()`，不新增 `run_awaitable_sync`，不新增同步函数包装 awaitable。
...
P3 超大文件队列当前仍只剩 `api/routes.py`，行数为 1470。剩余显式路由为 `/chat` 和
`/health`；`/health` 收益很低且承担多处父模块哨兵作用，不优先拆。下一候选边界应继续
...
## 2026-06-23 普通 API Chat Streaming Helper 拆分

状态：设计、计划、红灯测试、新模块拆分、父模块接入、验证和实现阶段提交均已完成。本阶段继续拆普通 `api/routes.py`，新增 `api/chat_streaming_helpers.py`，迁移 `_stream_chat()` 内的连续 delta 合并、ready queue drain 和断连后台 bounded queue drain helper。`api.routes` 继续保留 `/chat` 路由本体、`StreamingResponse`、`CHAT_STREAM_QUEUE_MAXSIZE`、`_normalize_chat_stream_event()`、`_chat_sse_data()`、`_stream_error_event()`、`_persist_stream_result_after_runner_done()`、断连后台落库、QQ push envelope、private buffer finalize 和 `/health`。`api/routes.py` 从 1449 行降至 1408 行；`api/chat_streaming_helpers.py` 为 81 行，拆分测试为 122 行。

设计文档：
`docs/superpowers/specs/2026-06-23-api-chat-streaming-helper-split-design.md`。
...
- [x] 汇总 `/chat` 剩余边界审计结论并确定 Chat Streaming Helper 边界。
- [x] 写入设计文档并提交。
- [x] 写入实现计划并提交。
- [x] 补普通 API chat streaming helper split 红灯测试并提交。
- [x] 新增 `api/chat_streaming_helpers.py`，锁定 `StreamEventCoalescer`、`collect_ready_stream_events()`、`drain_stream_queue_until_task_done()` 和不导入父模块约束，并提交。
- [x] 将 `_stream_chat()` 内联 pending delta / ready drain 逻辑改为调用新 helper，将 `_drain_stream_queue_until_runner_done()` 改为委托新模块，并提交。
- [x] 更新 `docs/todo.md`、本 walkthrough 和计划执行记录，完成最终验证后提交文档收口。
...
- 新增 split 红灯：
  ...
  -> `6 failed, 1 warning`；失败点为 `api/chat_streaming_helpers.py` 不存在。
- 相邻扫描红灯：
  `python -B -m pytest -p no:cacheprovider tests/test_api_history_log_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_agent_step_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_group_message_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_sticker_media_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable -v`
  -> `4 failed, 1 warning`，4 个失败均为新模块尚未创建。
- 新模块 split 绿灯：
...
- 相邻扫描绿灯：
  ...
  -> `4 passed, 1 warning`。
- streaming 行为回归：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_streaming_helpers_split.py tests/test_streaming_api.py tests/test_streaming_response_envelope.py -v`
  -> `15 passed, 21 warnings`。
- 断连后台与 asyncio 策略回归：
  ...
  -> `7 passed, 1 warning`。
- 行数检查：`api/routes.py` 1408 行，`api/chat_streaming_helpers.py` 81 行，`tests/test_api_chat_streaming_helpers_split.py` 122 行。
- 全量：`python -B -m pytest -p no:cacheprovider tests/ -v` ->
  `1722 passed, 6 skipped, 139 warnings in 131.90s`。

执行约束：

- 不拆 `/chat` 路由本体。
- 不拆 `/health`。
- 不迁移 `StreamingResponse`、`CHAT_STREAM_QUEUE_MAXSIZE`、SSE wrapper、streaming result holder、断连后台落库、QQ push envelope、private buffer finalize、`get_bridge`、`get_guardrail` 或聊天落库。
- 保持 `done.answer` / `done.reply` 最终权威语义来自 `bridge.handle_message()` 返回值。
- 不改变 Prompt Runtime 模板、`enriched_query`、conversation 结构、工具输出契约或 message envelope。
- 不新增 `asyncio.run()`，不新增 `run_awaitable_sync`，不新增同步函数包装 awaitable。

下一步：

P3 超大文件队列当前仍只剩 `api/routes.py`，行数为 1408。剩余显式路由为 `/chat` 和 `/health`；`/health` 收益很低且承担多处父模块哨兵作用，不优先拆。下一候选边界应继续围绕 `/chat` 主链路做细粒度设计：优先评估私聊缓冲基础件，继续保留父模块 monkeypatch facade，避免一次性迁移完整 `proxy_chat()`。

## 2026-06-23 普通 API Chat Private Buffer 基础件拆分

状态：设计、计划、红灯测试、新模块拆分、父模块接入、验证和实现阶段提交均已完成。本阶段继续拆普通 `api/routes.py`，新增 `api/chat_private_buffer.py`，迁移私聊缓冲的配置对象、owner / follower 状态结果、消息合并、文件合并、窗口计算和裸 dict 状态转移。`api.routes` 继续保留 `/chat` 路由本体、PrivateTimingGate、guardrail provider、Bridge 调用、owner deadline sleep、聊天落库、SSE、push envelope、response envelope、`_private_buffers`、`_private_lock` 和 `/health`。`api/routes.py` 从 1408 行降至 1351 行；`api/chat_private_buffer.py` 为 138 行，拆分测试为 184 行。

设计文档：
`docs/superpowers/specs/2026-06-23-api-chat-private-buffer-split-design.md`。
...
- 文档收口提交：随本次 `docs(计划): 收口私聊缓冲拆分` 完成。

计划列表：

- [x] 汇总 `/chat` 私聊缓冲边界审计结论并确定基础件拆分范围。
- [x] 写入设计文档并提交。
- [x] 写入实现计划并提交。
- [x] 补普通 API chat private buffer split 红灯测试并提交。
- [x] 新增 `api/chat_private_buffer.py`，锁定 `PrivateBufferConfig`、`PrivateBufferStore`、owner / follower 状态结果、纯 helper 和不导入父模块约束，并提交。
- [x] 将父模块 `_join_buffered_messages()`、`_merge_buffered_files()`、`_private_buffer_window_seconds()`、`_finalize_private_buffer()` 和 `proxy_chat()` 私聊缓冲分支改为委托新模块，并提交。
- [x] 更新 `docs/todo.md`、本 walkthrough 和计划执行记录，完成最终验证后提交文档收口。
...
- 新增 split 红灯：
  ...
  -> `4 failed, 1 passed, 1 warning`；失败点为 `api/chat_private_buffer.py` 不存在。
- 相邻扫描红灯：
  `python -B -m pytest -p no:cacheprovider tests/test_api_history_log_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_agent_step_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_group_message_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_sticker_media_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable -v`
  -> `4 failed, 1 warning`，4 个失败均为新模块尚未创建。
- 新模块 split 绿灯：
...
- 接入后扫描绿灯：
  `python -B -m pytest -p no:cacheprovider tests/test_api_history_log_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_agent_step_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_group_message_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_sticker_media_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable -v`
  -> `4 passed, 1 warning`。
- 行数检查：`api/routes.py` 1351 行，`api/chat_private_buffer.py` 138 行，`tests/test_api_chat_private_buffer_split.py` 184 行。
- 全量：`python -B -m pytest -p no:cacheprovider tests/ -v` ->
  `1727 passed, 6 skipped, 139 warnings in 129.45s`。
- 文档收口自检：
...
- 不拆 `/chat` 路由本体。
- 不拆 `/health`。
- 不迁移 PrivateTimingGate、guardrail provider、Bridge 调用、owner deadline sleep、聊天落库、SSE、push envelope 或 response envelope。
- 保持 `api.routes._private_buffers` 和 `api.routes._private_lock` 作为实际 runtime 状态与锁。
- 保持 `PRIVATE_BUFFER_WINDOW_SECONDS`、`PRIVATE_BUFFER_WINDOW_WITH_FILES_SECONDS`、`PRIVATE_BUFFER_FOLLOWER_TIMEOUT_SECONDS`、`MAX_BUFFERED_MESSAGES`、`asyncio.sleep`、`_time.time`、`get_guardrail`、`_detect_guardrail`、`get_bridge` 和 `_persist_chat_turn` patch point。
- 本阶段不引入 generation id，`_finalize_private_buffer(user_id)` 仍保持 user-level 语义。
- 不改变 Prompt Runtime 模板、`enriched_query`、conversation 结构、工具输出契约或 message envelope。
- 不新增 `asyncio.run()`，不新增 `run_awaitable_sync`，不新增同步函数包装 awaitable。

下一步：

P3 超大文件队列当前仍只剩 `api/routes.py`，行数为 1351。剩余显式路由为 `/chat` 和 `/health`；`/health` 收益很低且承担多处父模块哨兵作用，不优先拆。下一候选边界应继续围绕 `/chat` 主链路做细粒度设计，优先评估 response / push envelope 组装、owner deadline loop 或 streaming finalizer 的薄 helper，继续保留父模块 monkeypatch facade，避免一次性迁移完整 `proxy_chat()`。

## 2026-06-23 普通 API Chat Push Envelope 拆分

状态：设计、计划、红灯测试、新模块拆分、父模块接入、验证和实现阶段提交均已完成。本阶段继续拆普通 `api/routes.py`，新增 `api/chat_push_envelope.py`，迁移断连后台 push envelope 纯组装和传输层图片 token 展开 helper。`api.routes` 继续保留 `/chat` 路由本体、SSE 主循环、stream finalizer、`push_envelope_to_qq()` 调用点、DB 持久化、`_chat_response_payload()`、`get_bridge()`、`BackgroundTasks.add_task()` 和父模块 patch point；未迁移完整 `_stream_chat()`、`StreamingResponse` 或 `_persist_stream_result_after_runner_done()`。`api/routes.py` 从 1351 行降至 1331 行；`api/chat_push_envelope.py` 为 68 行，拆分测试为 120 行。

设计文档：
`docs/superpowers/specs/2026-06-23-api-chat-push-envelope-split-design.md`。

实现计划：
`.Codex/plans/api-chat-push-envelope-split.md`。

阶段提交：

- 设计提交：`e5c546a docs(普通API): 设计聊天推送信封拆分`。
- 计划提交：`084efb0 docs(计划): 记录聊天推送信封计划`。
- 红灯测试提交：`0148204 test(普通API): 锁定聊天推送信封契约`。
- 新模块提交：`da6fd04 refactor(普通API): 增加聊天推送信封助手`。
- 父模块接入提交：`52fa839 refactor(普通API): 接入聊天推送信封助手`。
- 文档收口提交：随本次 `docs(计划): 收口聊天推送信封拆分` 完成。

计划列表：

- [x] 汇总 `/chat` 断连后台 push envelope 与图片 token 传输展开边界。
- [x] 写入设计文档并提交。
- [x] 写入实现计划并提交。
- [x] 补普通 API chat push envelope split 红灯测试并提交。
- [x] 新增 `api/chat_push_envelope.py`，锁定 `ChatPushEnvelope`、`build_chat_push_envelope()`、`expand_chat_transport_answer()` 和不导入父模块约束，并提交。
- [x] 将断连后台 push 手写 envelope meta、流式 done 图片展开和非流式图片展开改为委托新模块，并提交。
- [x] 删除未使用的 `_chat_response_meta()` 父模块 dead facade。
- [x] 更新 `docs/todo.md`、本 walkthrough 和计划执行记录，完成最终验证后提交文档收口。
...
- 新增 split 红灯：
  ...
  -> `5 failed, 1 warning`；失败点为 `api/chat_push_envelope.py` 和父模块 wrapper 不存在。
- 相邻扫描红灯：
  `python -B -m pytest -p no:cacheprovider tests/test_api_history_log_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_agent_step_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_group_message_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_sticker_media_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable -v`
  -> `4 failed, 1 warning`，4 个失败均为新模块尚未创建。
- 新模块阶段验证：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_push_envelope_split.py -v`
  -> `4 passed, 1 failed, 1 warning`；失败点只剩父模块 wrapper，符合阶段边界。
- 父模块接入组合：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_push_envelope_split.py tests/test_api_push_envelope.py tests/test_chat_response_envelope.py tests/test_streaming_response_envelope.py -v`
...
- 扫描与 `asyncio.run` 策略验证：
  ...
  -> `7 passed, 1 warning`。
- 死 facade 清理证据：
  `rg -n "_chat_response_meta" api tests docs/superpowers/specs/2026-06-23-api-chat-push-envelope-split-design.md .Codex/plans/api-chat-push-envelope-split.md`
  -> 无输出，命令退出码为 1，表示无命中。
- 行数检查：`api/routes.py` 1331 行，`api/chat_push_envelope.py` 68 行，`tests/test_api_chat_push_envelope_split.py` 120 行。
- 全量：`python -B -m pytest -p no:cacheprovider tests/ -v` ->
...
- 不拆 `/chat` 路由本体。
- 不拆 `/health`。
- 不迁移完整 `_stream_chat()`、`StreamingResponse`、`_persist_stream_result_after_runner_done()`、stream result holder、DB 持久化、Bridge 调用或 `BackgroundTasks.add_task()`。
- 保持 `push_envelope_to_qq()` 实际调用点在 `api.routes` 内。
- 保持 `_chat_response_payload()`、`get_bridge()`、`_persist_chat_turn()`、`_finalize_private_buffer()` 和其他父模块 patch point。
- 不改变 Prompt Runtime 模板、`enriched_query`、conversation 结构、工具输出契约或 message envelope。
- 不新增 `asyncio.run()`，不新增 `run_awaitable_sync`，不新增同步函数包装 awaitable。
...
## 2026-06-23 普通 API Chat Media Precache 拆分

状态：设计、计划、红灯测试、新模块拆分、父模块接入、最终验证和阶段提交均已完成。本阶段把 `api.routes._schedule_image_precache()` 的调度实现拆到 `api/chat_media_precache.py`，父模块保留同名薄 wrapper。`proxy_chat()` 调用点、`_normalize_files` patch point、`BackgroundTasks.add_task()` 语义、图片预缓存懒加载、Bridge、私聊缓冲、guardrail、落库、SSE、push envelope、response envelope 和 `/health` 均保持原位。`api/routes.py` 从 1236 行降至 1233 行；`api/chat_media_precache.py` 为 34 行，拆分测试为 115 行。

设计文档：
`docs/superpowers/specs/2026-06-23-api-chat-media-precache-split-design.md`。

实现计划：
`.Codex/plans/api-chat-media-precache-split.md`。

阶段提交：

- 设计提交：`b2a0660 docs(普通API): 设计聊天图片预缓存拆分`。
- 计划提交：`b5df01e docs(计划): 记录聊天图片预缓存计划`。
- 红灯测试提交：`e1b3141 test(普通API): 锁定聊天图片预缓存契约`。
- 新模块提交：`a82bc18 refactor(普通API): 增加聊天图片预缓存助手`。
- 父模块接入提交：`73af90f refactor(普通API): 接入聊天图片预缓存助手`。
- 文档收口提交：随本次 `docs(计划): 收口聊天图片预缓存拆分` 完成。

计划列表：

- [x] 确认 `/chat` 图片预缓存拆分范围，排除 `/chat` 路由本体、Bridge、私聊缓冲、落库、SSE、push envelope 和 response envelope 迁移。
- [x] 写入设计文档并提交。
- [x] 写入实现计划并提交。
- [x] 补普通 API Chat Media Precache split 红灯测试并提交。
- [x] 新增 `api/chat_media_precache.py`，锁定 `schedule_image_precache()`、no-op、`BackgroundTasks.add_task()` 和不导入父模块约束，并提交。
- [x] 将父模块 `_schedule_image_precache()` 改为委托新模块的薄 wrapper，并提交。
- [x] 更新 `docs/todo.md`、本 walkthrough 和计划执行记录，完成最终验证后提交文档收口。
...
- 红灯：
  ...
  -> `4 failed, 1 warning`；失败点为 `api/chat_media_precache.py` 不存在或无法导入。
- 新模块阶段：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_media_precache_split.py -v`
  -> `1 failed, 3 passed, 1 warning`；唯一失败为父模块 wrapper 尚未委托新模块。
- 父模块接入定向绿灯：
...
- 静态检查：
  ...
  -> `1233 api/routes.py`、`34 api/chat_media_precache.py`、`115 tests/test_api_chat_media_precache_split.py`；
...
- 不拆 `/chat` 路由本体。
- 不拆 `/health`。
- 不迁移 `StreamingResponse`、stream finalizer、Bridge 调用、guardrail、私聊缓冲、聊天落库、push envelope 或 response envelope。
- 保持 `api.routes._schedule_image_precache()` 为父模块 wrapper，保留旧 monkeypatch 与调用边界。
- 父模块 wrapper 继续传入 `_normalize_files`，让旧 patch point 仍能影响实际调度。
- 新模块不导入 `api.routes`，且不在顶层导入 `nanobot_kt.image_pipeline`。
- 不改变 Prompt Runtime 模板、`enriched_query`、conversation 结构、工具输出契约或 message envelope。
- 不新增 `asyncio.run()`，不新增 `run_awaitable_sync`，不新增同步函数包装 awaitable。
...
## 2026-06-23 普通 API Chat Streaming Result 拆分

状态：设计、计划、红灯测试、新模块拆分、父模块接入、最终验证和阶段提交均已完成。本阶段把 `_stream_chat()` 中 stream runner 完成后的落库、private buffer finalize、Prompt V2 audit no-send 和断连后台 push 收尾逻辑拆到 `api/chat_streaming_result.py`。父模块继续保留 `/chat` route、完整 `_stream_chat()` SSE 主循环、`StreamingResponse`、`CHAT_STREAM_QUEUE_MAXSIZE`、done / error event、evolution trigger 和全部 monkeypatch facade。`api/routes.py` 从 1227 行降至 1163 行；`api/chat_streaming_result.py` 为 164 行，拆分测试为 237 行。

设计文档：
`docs/superpowers/specs/2026-06-23-api-chat-streaming-result-split-design.md`。
...
- 设计提交：`7b003e2 docs(普通API): 设计流式结果收尾拆分`。
- 计划提交：`658fe1a docs(计划): 记录流式结果收尾计划`。
- 红灯测试提交：`6db5958 test(普通API): 锁定流式结果收尾契约`。
- 新模块提交：`e39f8d7 refactor(普通API): 增加流式结果收尾助手`。
- 父模块接入提交：`49c1f2c refactor(普通API): 接入流式结果收尾助手`。
- 文档收口提交：随本次 `docs(计划): 收口流式结果收尾拆分` 完成。

计划列表：

- [x] 确认 Chat Streaming Result 拆分范围，排除完整 `_stream_chat()`、`proxy_chat()`、`StreamingResponse`、SSE 主循环和 message envelope 迁移。
- [x] 写入设计文档并提交。
- [x] 写入实现计划并提交。
- [x] 补普通 API Chat Streaming Result split 红灯测试和 chat split module 扫描约束，并提交。
- [x] 新增 `api/chat_streaming_result.py`，锁定 callbacks / context、成功落库、Prompt V2 audit no-send、后台 `UnitOfWork`、drain 和不导入父模块约束，并提交。
- [x] 将父模块 `_persist_stream_result_after_runner_done()` 改为委托新模块的薄 wrapper，并提交。
- [x] 更新 `docs/todo.md`、本 walkthrough 和计划执行记录，完成最终验证后提交文档收口。
...
- 红灯：
  ...
  -> `8 failed, 1 warning`；失败点为 `api/chat_streaming_result.py` 不存在或无法导入。
- 新模块定向：
...
- 父模块接入定向：
  ...
  -> `8 passed, 1 warning`。
- Streaming 相邻回归：
  `python -B -m pytest -p no:cacheprovider tests/test_streaming_api.py tests/test_streaming_response_envelope.py tests/test_api_chat_streaming_helpers_split.py tests/test_api_chat_push_envelope_split.py tests/test_chat_response_envelope.py tests/test_asyncio_run_policy.py -v`
  -> `27 passed, 21 warnings in 8.81s`。
- 静态检查：
  ...
  -> `1163 api/routes.py`、`164 api/chat_streaming_result.py`、`237 tests/test_api_chat_streaming_result_split.py`；
...
- 全量：
  `python -B -m pytest -p no:cacheprovider tests/ -v`
  -> `1758 passed, 6 skipped, 139 warnings in 122.37s (0:02:02)`。

执行约束：

- 不拆 `/chat` 路由本体。
- 不拆 `/health`。
- 不迁移完整 `_stream_chat()`、SSE 主循环、`StreamingResponse`、`CHAT_STREAM_QUEUE_MAXSIZE`、done / error event 或 evolution trigger。
- 保持 `api.routes._persist_stream_result_after_runner_done()` 为 `_stream_chat()` 内部薄 wrapper，父模块继续注入 `_persist_chat_turn()`、`_finalize_private_buffer()`、`_pop_bridge_reply_meta()`、`_private_prompt_audit_failure_meta()`、`_expand_chat_transport_answer()` 和 `_build_chat_push_envelope()`。
- 保持 request DB 与后台 `UnitOfWork` 新 session 的区别。
- 保持 Prompt V2 audit 失败不 push、assistant no-context meta、断连后台 push 和 bounded queue drain 语义。
- 新模块不导入 `api.routes`，不直接导入或调用 `push_envelope_to_qq`。
- 不改变 Prompt Runtime 模板、`enriched_query`、conversation 结构、工具输出契约或 message envelope。
- 不新增 `asyncio.run()`，不新增 `run_awaitable_sync`，不新增同步函数包装 awaitable。

下一步：

P3 超大文件队列当前仍只剩 `api/routes.py`，行数为 1163。剩余显式路由为 `/chat` 和 `/health`；`/health` 收益很低且承担多处父模块哨兵作用，不优先拆。下一候选边界建议先评估完整 SSE runtime 生成器能否独立成低耦合 helper；如果风险偏高，则改拆私聊 pre-bridge 决策、guardrail superuser 或 buffer flow 中不触碰 Bridge / 落库 / SSE 的纯状态协调 helper。继续保留父模块 monkeypatch facade，避免一次性迁移完整 `proxy_chat()`。

## 2026-06-23 普通 API Chat SSE Loop 拆分

状态：设计、计划、红灯测试、新模块拆分、父模块接入、最终验证和阶段提交均已完成。本阶段把 `_stream_chat()` 中纯 SSE queue pump 拆到 `api/chat_sse_loop.py`，负责等待 `stream_queue` / `done`、heartbeat、delta coalescing、tail drain 和 pending delta flush。父模块继续保留 `/chat` route、`StreamingResponse`、`CHAT_STREAM_QUEUE_MAXSIZE`、queue 创建、runner task、`bridge.handle_message(..., stream_queue=..., stream=True)`、业务收尾、Prompt V2 audit、success done payload、evolution trigger 和断连后台调度。`api/routes.py` 从 1163 行降至 1121 行；`api/chat_sse_loop.py` 为 100 行，拆分测试为 128 行。

设计文档：
`docs/superpowers/specs/2026-06-23-api-chat-sse-loop-split-design.md`。
...
- 设计提交：`da2f761 docs(普通API): 设计聊天 SSE 循环拆分`。
- 计划提交：`7be601d docs(计划): 记录聊天 SSE 循环计划`。
- 红灯测试提交：`ac3fe70 test(普通API): 锁定聊天 SSE 循环契约`。
- 新模块提交：`5696b15 refactor(普通API): 增加聊天 SSE 循环助手`。
- 父模块接入提交：`265f1c9 refactor(普通API): 接入聊天 SSE 循环助手`。
- 文档收口提交：随本次 `docs(计划): 收口聊天 SSE 循环拆分` 完成。

计划列表：

- [x] 确认 Chat SSE Loop 拆分范围，排除完整 `_stream_chat()`、`proxy_chat()`、`StreamingResponse`、业务收尾、message envelope 和 push envelope 迁移。
- [x] 写入设计文档并提交。
- [x] 写入实现计划并提交。
- [x] 补普通 API Chat SSE Loop split 红灯测试和 chat split module 扫描约束，并提交。
- [x] 新增 `api/chat_sse_loop.py`，锁定 heartbeat、done 快速结束、delta / progress 顺序、tail flush 和不导入父模块约束，并提交。
- [x] 将父模块 `_stream_chat()` 的手写 SSE 主循环改为委托 `chat_sse_loop.iter_chat_stream_events()`，并提交。
- [x] 更新 `docs/todo.md`、本 walkthrough 和计划执行记录，完成最终验证后提交文档收口。
...
- 红灯：
  ...
  -> `9 failed, 1 warning`；失败点为 `api/chat_sse_loop.py` 不存在或无法导入，父模块尚未委托新 loop。
- 新模块阶段：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_sse_loop_split.py -v`
  -> `1 failed, 4 passed, 1 warning`；4 个新模块行为测试通过，唯一失败为父模块尚未导入并委托 `chat_sse_loop`。
- 父模块接入组合回归：
  ...
  -> `39 passed, 21 warnings in 9.63s`。
- 静态检查：
  ...
  -> `1121 api/routes.py`、`100 api/chat_sse_loop.py`、`128 tests/test_api_chat_sse_loop_split.py`；
...
- 不拆 `/chat` 路由本体。
- 不拆 `/health`。
- 不迁移完整 `_stream_chat()`、`proxy_chat()`、`StreamingResponse`、`CHAT_STREAM_QUEUE_MAXSIZE`、queue 创建、runner task、Bridge 调用、业务收尾、Prompt V2 audit、done / error event、evolution trigger 或断连后台调度。
- 保持 `_normalize_chat_stream_event()`、`_chat_sse_data()`、`_stream_error_event()`、`_chat_response_payload()`、`_persist_chat_turn()`、`_finalize_private_buffer()` 和 `_pop_bridge_reply_meta()` 等父模块 facade。
- 新模块只把 queue raw event 转换为规范化 event dict，不产生 SSE 字符串。
- 新模块不导入 `api.routes`、FastAPI、DB、`core.daily_digest`、`get_bridge()` 或 `get_guardrail()`。
...
状态：设计、计划、红灯测试、helper 拆分、父模块接入、相邻回归、全量验证和阶段提交均已完成。本阶段把 `proxy_chat()` 中进入 Bridge 前的 private timing、guardrail 和 private buffer 决策编排拆到 `api/chat_pre_bridge_decision.py`。新模块只返回 `ChatPreBridgeEarlyReturn` 或 `ChatPreBridgeContinue`，父模块继续保留 DB、HTTP response、`_persist_chat_turn()`、`_chat_response_payload()`、guardrail silent 落库、`PersonaInjectionService`、Prompt Runtime payload、Bridge、SSE、非流式结果收尾和全部 monkeypatch facade。`api/routes.py` 从 1098 行降至 1022 行；`api/chat_pre_bridge_decision.py` 为 316 行，拆分测试为 378 行。

设计文档：
`docs/superpowers/specs/2026-06-24-api-chat-pre-bridge-decision-split-design.md`。
...
- 计划提交：`6bd3d86 docs(计划): 记录私聊前置决策计划`。
- 红灯测试提交：`2df1f9f test(普通API): 锁定私聊前置决策契约`。
- Helper 提交：`b56acbd refactor(普通API): 增加私聊前置决策助手`。
- 父模块接入提交：`aaba92c refactor(普通API): 接入私聊前置决策助手`。
- 验证记录提交：`cbb5ef3 docs(计划): 记录私聊前置决策验证`。
- 文档收口提交：随本次 `docs(计划): 收口私聊前置决策拆分` 完成。

计划列表：

- [x] 确认私聊 pre-bridge 决策拆分范围，排除 DB、HTTP response、Prompt Runtime、Bridge、SSE、message envelope 和 push envelope 迁移。
- [x] 写入设计文档并提交。
- [x] 写入实现计划并提交。
- [x] 补普通 API Chat 私聊 Pre-Bridge split 红灯测试和 chat split module 扫描约束，并提交。
- [x] 新增 `api/chat_pre_bridge_decision.py`，锁定 services、early / continue outcome、private timing、private buffer、guardrail 和不导入父模块约束，并提交。
- [x] 将父模块 `proxy_chat()` 中的 private timing / guardrail / private buffer 编排改为委托 `_resolve_chat_pre_bridge_decision()`，并提交。
- [x] 运行 chat split 扫描、private buffer / guardrail 相邻回归、静态检查和全量测试，记录验证结果。
- [x] 更新 `docs/todo.md`、本 walkthrough 和计划执行记录，完成最终验证后提交文档收口。

验证记录：

- 红灯：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_pre_bridge_decision_split.py tests/test_api_group_message_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_agent_step_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_history_log_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_sticker_media_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable -v`
  -> `12 failed, 1 warning in 6.94s`；失败点为 `api/chat_pre_bridge_decision.py` 不存在、父模块 wrapper 尚不存在、4 个 chat split module 扫描清单读取新模块失败。
- Helper 阶段：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_pre_bridge_decision_split.py -v`
  -> `6 passed, 2 failed, 1 warning in 6.57s`；新模块行为测试通过，剩余失败为父模块尚未提供 `_resolve_chat_pre_bridge_decision` 和 `_chat_pre_bridge_services`。
- 父模块接入定向：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_pre_bridge_decision_split.py -v`
  -> `8 passed, 1 warning in 0.96s`。
- Split 扫描：
  `python -B -m pytest -p no:cacheprovider tests/test_api_group_message_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_agent_step_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_history_log_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_sticker_media_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable -v`
  -> `4 passed, 1 warning in 1.11s`。
- Private buffer / guardrail 相邻回归：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_private_buffer_split.py tests/test_api_chat_guardrail_facade_split.py tests/test_api.py::test_proxy_chat_persists_private_timing_scoring_meta tests/test_api.py::test_proxy_chat_no_reply_persists_private_timing_scoring_meta tests/test_api.py::test_private_buffer_silent_releases_waiters tests/test_api.py::test_private_buffer_refreshes_window_and_persists_merged_messages tests/test_api.py::test_private_buffer_merges_files_for_final_bridge_request tests/test_api.py::test_private_buffer_text_after_files_shrinks_window_to_five_seconds tests/test_api.py::test_private_buffer_owner_cancel_releases_waiters_and_cleans_buffer tests/test_api.py::test_private_buffer_bridge_cancel_releases_waiters_and_cleans_buffer tests/test_asyncio_run_policy.py -v`
  -> `27 passed, 21 warnings in 5.18s`。
- 静态检查：
  `python -m compileall api/routes.py api/chat_pre_bridge_decision.py -q` 退出码 0；
  `git diff --check -- ...` 无输出；
  `wc -l api/routes.py api/chat_pre_bridge_decision.py tests/test_api_chat_pre_bridge_decision_split.py`
  -> `1022 api/routes.py`、`316 api/chat_pre_bridge_decision.py`、`378 tests/test_api_chat_pre_bridge_decision_split.py`。
- 全量：
  `python -B -m pytest -p no:cacheprovider tests/ -v`
  -> `1776 passed, 6 skipped, 139 warnings in 137.28s (0:02:17)`。
- 文档收口提交前复跑：
  ...
  -> `1776 passed, 6 skipped, 139 warnings in 123.02s (0:02:03)`。

async 策略核对：

- 本阶段新模块和 `api/routes.py` 未新增 `asyncio.run`、`run_awaitable_sync` 或同步函数包装 awaitable。
- repo-wide `rg -n "asyncio\\.run|run_awaitable_sync" api core clients nanobot_kt tests --glob '*.py'` 仍命中既有 `core/async_bridge.py`、`core/evolution.py`、`core/eval_sampling/scheduler.py` 和测试策略文件；本阶段未扩大范围。

执行约束：

- 不拆 `/chat` 路由本体。
- 不拆 `/health`。
- 不迁移 `_build_chat_context()`、history 注入、`PersonaInjectionService`、Prompt Runtime payload、`safe_user_input`、`enriched_query`、`bridge_meta`、Bridge、SSE、message envelope 或 push envelope。
- 父模块继续负责 early outcome 落库和 response、guardrail silent 落库和 response、`_clone_chat_request()`、Prompt Runtime、Bridge 调用和后续非流式 / 流式收尾。
- 新模块只通过 `ChatPreBridgeServices` 接收 `_private_buffer_store`、`_private_buffer_config()`、`asyncio.sleep`、`_time.time`、`_wait_private_buffer_deadline()`、`_finalize_private_buffer()`、`get_guardrail()`、`_detect_guardrail()`、`get_private_gate()`、`get_casual_reply()` 和 logger。
- 新模块不导入 `api.routes`、FastAPI、DB、Bridge 或 Prompt Runtime。
- 不改变 Prompt Runtime 模板、`enriched_query`、conversation 结构、工具输出契约、message envelope、push envelope 或 response envelope。
- 不新增 `asyncio.run()`，不新增 `run_awaitable_sync`，不新增同步函数包装 awaitable。
...
状态：设计、计划、红灯测试、helper 拆分、父模块接入、wrapper 压缩、相邻回归、全量验证和阶段提交均已完成。本阶段把 `proxy_chat()` 中 `ChatPreBridgeEarlyReturn` / `ChatPreBridgeContinue` 的 route 层转译拆到 `api/chat_pre_bridge_route_result.py`。父模块继续保留 HTTP route、DB session、persona injection、Prompt Runtime payload、Bridge、SSE、response envelope 和后续落库边界；新模块只通过 callbacks 复用 `_clone_chat_request()`、`_persist_chat_turn()`、`_chat_response_payload()` 和 `_finalize_private_buffer()` patch point。`api/routes.py` 从 1020 行降至 1013 行；`api/chat_pre_bridge_route_result.py` 为 115 行，拆分测试为 311 行。

设计文档：
`docs/superpowers/specs/2026-06-24-api-chat-pre-bridge-route-result-split-design.md`。
...
- 设计提交：`95e19bc docs(普通API): 设计前置决策结果拆分`。
- 计划提交：`2f8202c docs(计划): 记录前置决策结果计划`。
- 红灯测试提交：`db6fe23 test(普通API): 锁定前置决策结果契约`。
- Helper 提交：`6947503 refactor(普通API): 增加前置决策结果助手`。
- 父模块接入提交：`2c8a7a1 refactor(普通API): 接入前置决策结果助手`。
- Wrapper 压缩提交：`4794458 refactor(普通API): 压缩前置结果包装器`。
- 验证记录提交：`0abaab4 docs(计划): 记录前置决策结果验证`。
- 文档收口提交：随本次 `docs(计划): 收口前置决策结果拆分` 完成。

计划列表：

- [x] 确认 pre-bridge route result 拆分范围，排除 HTTP route、DB session、PersonaInjectionService、Prompt Runtime payload、Bridge、SSE、message envelope、push envelope 和 response envelope 迁移。
- [x] 写入设计文档并提交。
- [x] 写入实现计划并提交。
- [x] 补普通 API Chat Pre-Bridge Route Result split 红灯测试和 chat split module 扫描约束，并提交。
- [x] 新增 `api/chat_pre_bridge_route_result.py`，锁定 early return、continue、guardrail silent 和不导入父模块约束，并提交。
- [x] 将父模块 `proxy_chat()` 中的内联 route result 转译改为委托 `_resolve_pre_bridge_route_result()`，并提交。
- [x] 压缩父模块 wrapper 排版，确保 `api/routes.py` 行数继续下降，并提交。
- [x] 运行 chat split 扫描、相邻回归、静态检查和全量测试，记录验证结果。
- [x] 更新 `docs/todo.md`、本 walkthrough 和计划执行记录，完成最终验证后提交文档收口。
...
- 红灯：
  ...
  -> `10 failed, 1 warning in 6.84s`；失败点为 `api/chat_pre_bridge_route_result.py` 不存在、`api.chat_pre_bridge_route_result` 无法 import、父模块 `_chat_pre_bridge_route_callbacks()` / `_resolve_pre_bridge_route_result()` 尚不存在，以及 4 个 chat split module 扫描清单读取新模块失败。
- Helper 阶段：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_pre_bridge_route_result_split.py -v`
  -> `5 passed, 1 failed, 1 warning in 6.49s`；新模块行为测试通过，剩余失败为父模块尚未提供 `_chat_pre_bridge_route_callbacks()`。
- 父模块接入定向：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_pre_bridge_route_result_split.py -v`
...
- Pre-bridge / non-streaming / timing meta 相邻回归：
  ...
  -> `15 passed, 21 warnings in 2.36s`。
- 静态检查：
...
- 本阶段新模块和 `api/routes.py` 未新增 `asyncio.run`、`run_awaitable_sync` 或同步函数包装 awaitable。
- `rg -n "asyncio\\.run|run_awaitable_sync" api/routes.py api/chat_pre_bridge_route_result.py tests/test_api_chat_pre_bridge_route_result_split.py` 仅命中新测试中的禁止断言字符串，生产代码无新增命中。
...
- 不拆 `/health`。
- 不迁移 `_build_chat_context()`、history 注入、`PersonaInjectionService`、Prompt Runtime payload、`safe_user_input`、`enriched_query`、`bridge_meta`、Bridge、SSE、message envelope、push envelope 或 response envelope。
- 父模块继续负责 HTTP route、DB session、Prompt Runtime、Bridge 调用和后续非流式 / 流式收尾。
- 父模块通过 `_chat_pre_bridge_route_callbacks(db)` 用闭包把当前 `db` 绑定到 `_persist_chat_turn()`；helper 不导入 DB model 或父模块。
- guardrail silent 分支继续使用 `persist_req` 持久化 `（数据中转，自动静默）`。
- 新模块不导入 `api.routes`、FastAPI、Bridge、Prompt Runtime 或 DB 全局入口。
- 不改变 Prompt Runtime 模板、`enriched_query`、conversation 结构、工具输出契约、message envelope、push envelope 或 response envelope。
- 不新增 `asyncio.run()`，不新增 `run_awaitable_sync`，不新增同步函数包装 awaitable。
...
## 2026-06-24 普通 API Chat Route Runner 拆分

状态：设计、计划、红灯测试、helper 拆分、父模块接入、父模块压缩、Prompt Runtime 核查、相邻回归、全量验证和阶段提交均已完成。本阶段把 `proxy_chat()` 中 Bridge 调用后的 stream / non-streaming route runner 编排拆到 `api/chat_route_runner.py`，包括 SSE 事件产出、断连后台收尾登记、非流式结果收尾委托和 route 级错误描述。父模块继续保留 `/chat` HTTP route、鉴权、DB session、`get_bridge()` patch point、`StreamingResponse`、`HTTPException`、pre-bridge、Prompt Runtime payload、message envelope、push envelope 和 response envelope 边界。`api/routes.py` 从 1005 行降至 783 行；`api/chat_route_runner.py` 为 350 行，拆分测试为 432 行。当前 P3 超大文件队列已收口；全仓其他历史大文件未纳入本轮 P3 队列，后续如需治理应另立计划。

设计文档：
`docs/superpowers/specs/2026-06-24-api-chat-route-runner-split-design.md`。
...
- 设计提交：`a6f1e61 docs(普通API): 设计聊天路由执行器拆分`。
- 计划提交：`8f71059 docs(计划): 记录聊天路由执行器计划`。
- 红灯测试提交：`abb377a test(普通API): 锁定聊天路由执行器契约`。
- Helper 提交：`fb3ad93 refactor(普通API): 增加聊天路由执行器`。
- 父模块接入提交：`c0f3d70 refactor(普通API): 接入聊天路由执行器`。
- 父模块压缩提交：`aecfda9 refactor(普通API): 压缩聊天路由父模块`。
- 文档收口提交：随本次 `docs(计划): 收口聊天路由执行器拆分` 完成。

计划列表：

- [x] 确认 Chat Route Runner 拆分范围，排除 `/chat` route、鉴权、DB session、HTTP response / exception、Prompt Runtime payload、message envelope、push envelope 和 response envelope 迁移。
- [x] 写入设计文档并提交。
- [x] 写入实现计划并提交。
- [x] 补普通 API Chat Route Runner split 红灯测试和 chat split module 扫描约束，并提交。
- [x] 新增 `api/chat_route_runner.py`，锁定 streaming success / error / audit failed / disconnect、non-streaming success / error / cancellation / audit failed 和不导入父模块约束，并提交。
- [x] 将父模块 `_stream_chat()` / `_do_chat()` 的 route runner 编排改为委托 `chat_route_runner.iter_streaming_chat_response()` 与 `chat_route_runner.run_non_streaming_chat_response()`，并提交。
- [x] 压缩父模块 wrapper、删除未使用 import / 常量，让 `api/routes.py` 降至 800 行以下，并提交。
- [x] 运行 Prompt Runtime 核查、chat split 扫描、相邻回归、静态检查和全量测试，记录验证结果。
- [x] 更新 `docs/todo.md`、本 walkthrough 和计划执行记录，完成最终验证后提交文档收口。
...
- 红灯：
  ...
  -> `10 failed, 1 warning in 6.34s`；失败点为 `api/chat_route_runner.py` 不存在、`api.chat_route_runner` 无法 import、父模块 route runner 委托入口尚不存在。
- 扫描红灯：
  `python -B -m pytest -p no:cacheprovider tests/test_api_group_message_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_agent_step_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_history_log_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_sticker_media_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable -v`
  -> `4 failed, 1 warning in 6.83s`；四个失败均为扫描清单读取 `api/chat_route_runner.py` 时文件尚不存在。
- Helper 阶段：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_route_runner_split.py -k 'not parent_chat_route_delegates' -v`
  -> `9 passed, 1 deselected, 1 warning in 0.93s`；完整新测试为 `9 passed, 1 failed, 1 warning in 6.56s`，唯一失败为父模块尚未接入。
- 父模块接入定向：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_route_runner_split.py -v`
  -> `10 passed, 1 warning in 0.92s`。
- 相邻 chat split 回归：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_route_runner_split.py tests/test_api_chat_sse_loop_split.py tests/test_api_chat_streaming_result_split.py tests/test_api_chat_streaming_helpers_split.py tests/test_api_chat_non_streaming_result_split.py tests/test_api_chat_runtime_facade_split.py tests/test_api_chat_runtime_route_context_split.py -v`
  -> `45 passed, 21 warnings in 4.57s`。
- Split 扫描：
  `python -B -m pytest -p no:cacheprovider tests/test_api_group_message_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_agent_step_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_history_log_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable tests/test_api_sticker_media_routes_split.py::test_chat_split_modules_do_not_import_parent_routes_or_sync_awaitable -v`
  -> `4 passed, 1 warning in 0.96s`。
- 主 API 回归：
  `python -B -m pytest -p no:cacheprovider tests/test_api.py -v`
  -> `82 passed, 21 warnings in 22.08s`。
- Streaming / envelope 回归：
...
- Wrapper / patch point 回归：
  `python -B -m pytest -p no:cacheprovider tests/test_api_chat_route_runner_split.py tests/test_api_chat_helpers_split.py tests/test_api_chat_private_buffer_split.py tests/test_api_chat_persistence_split.py tests/test_api_chat_request_contract_split.py -v`
  -> `61 passed, 1 warning in 3.31s`。
- 静态检查：
  ...
  -> `783 api/routes.py`、`350 api/chat_route_runner.py`、`432 tests/test_api_chat_route_runner_split.py`。
- 全量：
  `env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY python -B -m pytest -p no:cacheprovider tests/ -v`
...
- 本阶段新模块和 `api/routes.py` 未新增 `asyncio.run`、`run_awaitable_sync` 或同步函数包装 awaitable。
- `rg -n "asyncio\\.run|run_awaitable_sync" api/routes.py api/chat_route_runner.py tests/test_api_chat_route_runner_split.py || true` 仅命中新测试中的禁止断言字符串，生产代码无新增命中。

Prompt Runtime 核查：

- `rg -n "enriched_query|<user_input>|bridge_meta|history_header|history_messages|raw_query|persona_text|PromptRuntime|prompt_v2" api/routes.py api/chat_route_runner.py prompts.v2.default/chat data/prompts_v2/chat core/prompt_v2/variables.py core/prompt_v2/template_registry.py`
  显示字段仍按原名透传或由既有模板引用；本阶段未改字段名、模板标记、变量语义、`enriched_query` 包裹、history / persona 注入、conversation 结构、工具输出契约、message envelope、push envelope 或 response envelope，因此默认模板和 `data/prompts_v2/` 运行时模板无需变更。

执行约束：

- 不拆 `/chat` 路由本体。
- 不拆 `/health`。
- 不迁移 `get_bridge()` patch point、`StreamingResponse`、`HTTPException`、DB session、pre-bridge、Prompt Runtime payload、message envelope、push envelope 或 response envelope。
- 新模块通过 `ChatRouteRunnerCallbacks` 接收父模块能力和外部服务，不导入 `api.routes`、FastAPI HTTP 边界、DB / UoW 全局入口、Bridge provider 或 QQ push provider。
- 父模块仍负责 HTTP 边界和错误转译；新模块只返回 route runner 结果或 `ChatRouteHttpError` 描述。
- Prompt Runtime 字段名、`<user_input>` 包裹语义、history / persona 注入、conversation 结构、工具输出契约、message envelope、push envelope 和 response envelope 均未改变。
- 不新增 `asyncio.run()`，不新增 `run_awaitable_sync`，不新增同步函数包装 awaitable。

下一步：

`docs/todo.md` 中 P3 超大文件队列当前已完成，下一项硬任务是「ruff 批量清理」。建议下一阶段先运行 `ruff check` 获取真实告警清单，按 F401 / F841 / E402 / F811 / 旧式泛型 / datetime 等类别分批修复，每批保持小提交并运行定向测试，最后再跑全量测试。
