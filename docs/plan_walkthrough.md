...
状态：P2-1 已完成设计、实现计划、任务 1、任务 2 和任务 3，待进入任务 4。`docs/todo.md` 路线项 4 明确当前痛点是工具策略缺少完整 platform 维度：现有工具裁剪已有 `chat_type`、`group`、`user`、`runtime_preset` 和后端 `platform` override 解析，真实入口也已把 `client_meta.platform` 传到 Bridge / ToolPlan；Admin API 和 WebUI 尚未完成平台覆盖配置。当前焦点校准为：入口透传已落地，下一步执行 Admin API platform override / effective preview / tools preview。
...
P2-1 工具 platform 维度配置已完成前三步：后端解析支持 `ToolOverride(scope_type="platform")`，运行时决策记录和迁移已带 `platform`，真实入口已把 `client_meta.platform` 透传到 Bridge、ToolPlan 和 runtime decision。下一步优先执行任务 4「Admin API 支持 platform 覆盖和预览」，之后继续任务 5 WebUI 平台覆盖配置和任务 6 文档收口。P2-2「标准化请求 / 响应信封」要等 P2-1 完成后再切入；TimingGate 真实日志标注 / CI 接入属于运营延续项，不抢占 P2 执行顺序。
