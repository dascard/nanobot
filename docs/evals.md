...
该入口只运行稳定 baseline gate。P4-5B 已完成周期性复跑和报告归档；P4-5C 已完成第一轮 RAG manual 样本扩充；P4-5D 已完成 memory fixture-backed positive RAG case；P4-5E 已补齐 knowledge fixture 引用正例；P4-5F 已补齐 sticker fixture sendable 正例。RAG stable gate 当前包含 9 个 manual constraint case 和 3 个固定 fixture positive case。
...
P4-4 已为 RAG benchmark 增加专用 baseline diff 和 gate。P4-5F 后，稳定门禁运行 manual case、固定 fixture positive cases 和 deterministic provider：
...
P4-5D 已新增 `evals/rag_benchmark/fixtures.py` 和 `positive_v1` fixture DB builder。P4-5F 已把 `positive_v1` 从单一 memory 正例扩展为 memory + knowledge + sticker 三正例：baseline 包含 `memory_fixture_positive_001`、`knowledge_fixture_positive_001` 和 `sticker_fixture_positive_001`，`metrics.overall.positive_cases=3`、`metrics.source:knowledge.positive_cases=1`、`metrics.source:sticker.positive_cases=1`、`hit@5=1.0`、`mrr=1.0`。knowledge fixture 固定命中 `knowledge:9001:chunk:0`，并通过 `requires_citation=true` 的 `checks.citation=true`；sticker fixture 固定命中 `sticker:9101:sticker`，并通过 `requires_sendable=true` 的 `checks.sendable=true`。PR gate 使用 `--min-hit-at-5 1.0` 和 `--min-mrr 1.0`，防止正例召回退化。
