...
- [ ] 纯 `directed_to_other` no_reply 语义仍被 prompt 和 eval case 保留。
- [ ] `@bot + @其他人` 与 `directed_to_other + linger` 不再被 prompt 描述为硬 no_reply。
- [ ] 内嵌 prompt 和 Prompt Runtime 默认模板同义。
- [ ] 新增 paired cases 走 `model_assisted_conflict`，不是 `rule_shortcut`。
- [ ] `scripts/run_timing_gate_gate.sh` 通过。
- [ ] 不修改 scoring 公式、不更新生产参数、不改变 PR gate 或周期 gate。
