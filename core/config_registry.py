...
def _legacy_setting_alias_projection() -> Mapping[str, LegacySettingAlias]:
    """从 Compatibility Registry 生成旧设置只读投影。"""

    projected: dict[str, LegacySettingAlias] = {}
    for descriptor in COMPATIBILITY_REGISTRY.descriptors(CompatibilityKind.SETTING):
        if descriptor.environment_alias is None:
            raise ValueError(f"旧设置 {descriptor.compatibility_id} 缺少环境变量 alias")
...
def _validate_proactive_outreach_probability_range(
    ...
    if float(minimum) > float(maximum):
        raise ValueError("proactive_outreach.surge_min_prob 不能大于 surge_max_prob")


def _validate_private_timing_confidence_thresholds(
    values: Mapping[str, object],
) -> None:
    decision = values.get("private_timing.decision_confidence_threshold")
    template = values.get("private_timing.template_confidence_threshold")
...
def _validate_news_governance_settings(
    ...
    mode = str(values.get("news.relevance_review.mode", "disabled")).strip().lower()
    if mode not in {"disabled", "observation", "active"}:
        raise ValueError(
            "news.relevance_review.mode 必须是 disabled/observation/active"
        )
...
def _validate_agent_runtime_settings(values: Mapping[str, object]) -> None:
    default_kind = (
        str(values.get("agent.runtime.default", "native") or "native").strip().lower()
    )
    if default_kind not in {"native", "kt"}:
        raise ValueError("agent.runtime.default 必须是 native 或 kt")
    kt_enabled = bool(values.get("agent.runtime.kt_enabled", False))
    basis_points = int(values.get("agent.runtime.kt_rollout_basis_points", 0) or 0)
    allowlist = str(values.get("agent.runtime.kt_session_allowlist", "") or "").strip()
    if "*" in allowlist or "?" in allowlist:
...
def _validate_context_compaction_settings(
    ...
    thresholds = tuple(int(values.get(key, default)) for key, default in (
        ("context.compaction.notice_tokens", 64_000),
        ("context.compaction.snip_tokens", 72_000),
        ("context.compaction.summary_tokens", 80_000),
        ("context.compaction.hard_limit_tokens", 96_000),
    ))
    if tuple(sorted(set(thresholds))) != thresholds:
        raise ValueError("Context 压缩水位必须严格满足 notice < snip < summary < hard")
    target = int(values.get("context.compaction.target_tokens", 68_000))
    if not 0 < target < thresholds[1]:
        raise ValueError("Context 压缩 target_tokens 必须小于 snip_tokens")
    inline_chars = int(values.get(
        "context.compaction.tool_inline_max_chars",
        12_000,
    ))
    head = int(values.get(
        "context.compaction.tool_snippet_head_chars",
        6_000,
    ))
    tail = int(values.get(
        "context.compaction.tool_snippet_tail_chars",
        2_000,
    ))
    if head + tail > inline_chars:
...
SETTING_DEFS: dict[str, SettingDef] = {
    ...
    "database.url": SettingDef(
        ...
        default="sqlite:///./data/nanobot.db",
        value_type="str",
        category="system",
        description="数据库连接地址",
        restart_required=True,
    ...
    "agent.runtime.default": SettingDef(
        key="agent.runtime.default",
        env_name="NANOBOT_AGENT_RUNTIME_DEFAULT",
        default="native",
        value_type="str",
        category="agent",
        description="主聊天默认 Agent Runtime：native/kt",
        restart_required=True,
        source_precedence=("environment", "default"),
    ...
    "agent.runtime.kt_enabled": SettingDef(
        key="agent.runtime.kt_enabled",
        env_name="NANOBOT_AGENT_RUNTIME_KT_ENABLED",
        default=False,
        value_type="bool",
        category="agent",
        description="是否允许主聊天选择可选 KT Runtime",
        restart_required=True,
        source_precedence=("environment", "default"),
        owner_module="runtime.agent",
        safety_class="invariant",
    ),
    "agent.runtime.kt_rollout_basis_points": SettingDef(
        key="agent.runtime.kt_rollout_basis_points",
        env_name="NANOBOT_AGENT_RUNTIME_KT_ROLLOUT_BPS",
        default=0,
        value_type="int",
        category="agent",
        description="KT 主聊天稳定哈希灰度比例，0..10000 个基点",
        restart_required=True,
        min_value=0,
        max_value=10_000,
        source_precedence=("environment", "default"),
    ...
    "agent.runtime.kt_session_allowlist": SettingDef(
        key="agent.runtime.kt_session_allowlist",
        env_name="NANOBOT_AGENT_RUNTIME_KT_SESSIONS",
        default="",
        value_type="str",
        category="agent",
        description="精确选择 KT Runtime 的 canonical session_id 列表",
        restart_required=True,
        source_precedence=("environment", "default"),
        owner_module="runtime.agent",
        safety_class="invariant",
    ...
    "agent.multi_agent.enabled": SettingDef(
        ...
        env_name="NANOBOT_MULTI_AGENT_ENABLED",
        default=False,
        value_type="bool",
        category="agent",
        description="是否启用冻结计划驱动的多 Agent 协作入口",
        restart_required=True,
        dangerous=True,
        source_precedence=("environment", "default"),
        owner_module="core.agent_collaboration",
        safety_class="invariant",
    ),
    "context.compaction.notice_tokens": SettingDef(
        key="context.compaction.notice_tokens",
        env_name="NANOBOT_CONTEXT_NOTICE_TOKENS",
        default=64_000,
        value_type="int",
        category="agent",
        description="Context 仅告警水位 token 数",
        restart_required=True,
        min_value=1_024,
        max_value=2_000_000,
        dangerous=True,
        owner_module="core.context_compaction",
        cross_field_validator=_validate_context_compaction_settings,
    ),
    "context.compaction.snip_tokens": SettingDef(
        key="context.compaction.snip_tokens",
        env_name="NANOBOT_CONTEXT_SNIP_TOKENS",
        default=72_000,
        value_type="int",
        category="agent",
        description="Context 摘录和整组裁剪水位 token 数",
        restart_required=True,
        min_value=1_025,
        max_value=2_000_000,
        dangerous=True,
    ...
    "context.compaction.summary_tokens": SettingDef(
        key="context.compaction.summary_tokens",
        env_name="NANOBOT_CONTEXT_SUMMARY_TOKENS",
        default=80_000,
        value_type="int",
        category="agent",
        description="Context 确定性历史摘要水位 token 数",
        restart_required=True,
        min_value=1_026,
        max_value=2_000_000,
        dangerous=True,
    ...
    "context.compaction.hard_limit_tokens": SettingDef(
        key="context.compaction.hard_limit_tokens",
        env_name="NANOBOT_CONTEXT_HARD_LIMIT_TOKENS",
        default=96_000,
        value_type="int",
        category="agent",
        description="Context 不可越过的硬上限 token 数",
        restart_required=True,
        min_value=1_027,
        max_value=2_000_000,
        dangerous=True,
    ...
    "context.compaction.target_tokens": SettingDef(
        key="context.compaction.target_tokens",
        env_name="NANOBOT_CONTEXT_TARGET_TOKENS",
        default=68_000,
        value_type="int",
        category="agent",
        description="压缩后目标 token 数",
        restart_required=True,
        min_value=512,
        max_value=1_999_999,
        dangerous=True,
    ...
    "context.compaction.recent_units_to_keep": SettingDef(
        key="context.compaction.recent_units_to_keep",
        env_name="NANOBOT_CONTEXT_RECENT_UNITS",
        default=6,
        value_type="int",
        category="agent",
        description="摘要时保留的近期原子消息组数",
        restart_required=True,
        min_value=1,
        max_value=128,
        owner_module="core.context_compaction",
    ),
    "context.compaction.snip_message_chars": SettingDef(
        key="context.compaction.snip_message_chars",
        env_name="NANOBOT_CONTEXT_SNIP_MESSAGE_CHARS",
        default=4_000,
        value_type="int",
        category="agent",
        description="单条旧消息确定性首尾摘录字符上限",
        restart_required=True,
        min_value=256,
        max_value=100_000,
        owner_module="core.context_compaction",
    ),
    "context.compaction.summary_chars": SettingDef(
        key="context.compaction.summary_chars",
        env_name="NANOBOT_CONTEXT_SUMMARY_CHARS",
        default=8_000,
        value_type="int",
        category="agent",
        description="确定性历史摘要字符上限",
        restart_required=True,
        min_value=512,
        max_value=200_000,
        owner_module="core.context_compaction",
    ),
    "context.compaction.tool_inline_max_bytes": SettingDef(
        key="context.compaction.tool_inline_max_bytes",
        env_name="NANOBOT_TOOL_RESULT_INLINE_BYTES",
        default=32 * 1024,
        value_type="int",
        category="agent",
        description="工具结果转 Artifact 前允许内联的字节上限",
        restart_required=True,
        min_value=1_024,
    ...
    "context.compaction.tool_inline_max_chars": SettingDef(
        key="context.compaction.tool_inline_max_chars",
        env_name="NANOBOT_TOOL_RESULT_INLINE_CHARS",
        default=12_000,
        value_type="int",
        category="agent",
        description="工具结果转 Artifact 前允许内联的字符上限",
        restart_required=True,
        min_value=512,
        max_value=4_000_000,
        dangerous=True,
    ...
    "context.compaction.tool_snippet_head_chars": SettingDef(
        key="context.compaction.tool_snippet_head_chars",
        env_name="NANOBOT_TOOL_RESULT_HEAD_CHARS",
        default=6_000,
        value_type="int",
        category="agent",
        description="Artifact 化工具结果保留的头部字符数",
        restart_required=True,
        min_value=128,
        max_value=2_000_000,
        owner_module="core.context_compaction",
    ...
    "context.compaction.tool_snippet_tail_chars": SettingDef(
        key="context.compaction.tool_snippet_tail_chars",
        env_name="NANOBOT_TOOL_RESULT_TAIL_CHARS",
        default=2_000,
        value_type="int",
        category="agent",
        description="Artifact 化工具结果保留的尾部字符数",
        restart_required=True,
        min_value=128,
        max_value=2_000_000,
        owner_module="core.context_compaction",
    ...
    "classifier.timeout": SettingDef(
        key="classifier.timeout",
        env_name="CLASSIFIER_TIMEOUT",
        default=15.0,
        value_type="float",
        category="classifier",
        description="TimingGate/分类器超时(秒)",
        min_value=1,
        max_value=120,
    ),
    ...
    "private_timing.template_confidence_threshold": SettingDef(
        ...
        max_value=1,
        owner_module="core.private_timing_policy",
        cross_field_validator=(_validate_private_timing_confidence_thresholds),
    ),
    ...
    "timing_gate.proactive.activity_floor": SettingDef(
        ...
        value_type="int",
        category="classifier",
        description="主动发言活跃度下限(msg_5m 少于此值视为死群,不主动)",
        min_value=0,
        max_value=100,
    ),
    "timing_gate.proactive.activity_ceiling": SettingDef(
        ...
        value_type="int",
        category="classifier",
        description="主动发言活跃度上限(msg_1m 超过此值视为刷屏,不插话)",
        min_value=1,
        max_value=500,
    ),
    "image_summary.timeout": SettingDef(
        key="image_summary.timeout",
        env_name="IMAGE_SUMMARY_TIMEOUT",
        default=120.0,
        value_type="float",
        category="image",
        description="图片摘要超时(秒)",
        min_value=1,
        max_value=300,
    ),
    "image_summary.max_tokens": SettingDef(
        key="image_summary.max_tokens",
        env_name="IMAGE_SUMMARY_MAX_TOKENS",
        default=512,
        value_type="int",
        category="image",
        description="图片摘要最大token",
        min_value=64,
        max_value=4096,
    ),
    "image_summary.temperature": SettingDef(
        key="image_summary.temperature",
        env_name="IMAGE_SUMMARY_TEMPERATURE",
        default=0.1,
        value_type="float",
        category="image",
        description="图片摘要temperature",
        min_value=0,
        max_value=2.0,
    ),
    "image_summary.top_p": SettingDef(
        key="image_summary.top_p",
        env_name="IMAGE_SUMMARY_TOP_P",
        default=0.9,
        value_type="float",
        category="image",
        description="图片摘要top_p",
        min_value=0,
        max_value=1.0,
    ),
    ...
    "sticker.auto_describe_max_per_cycle": SettingDef(
        ...
        value_type="int",
        category="sticker",
        description="每轮自动描述上限",
        min_value=1,
        max_value=100,
    ),
    "model.reply": SettingDef(
        key="model.reply",
        env_name="LLM_MODEL_REPLY",
        default="deepseek-v4-flash-max",
        value_type="str",
        category="model",
        description="对话回复模型ID",
        owner_module="core.model_provider",
    ),
    "model.reply_intel_floor": SettingDef(
        key="model.reply_intel_floor",
        env_name="REPLY_MODEL_INTEL_FLOOR",
        default=12,
        value_type="int",
        category="model",
        description="回复候选模型最低智能等级",
        min_value=1,
        max_value=20,
        owner_module="core.model_provider",
    ),
    "model.reply_intel_boost": SettingDef(
        key="model.reply_intel_boost",
        env_name="REPLY_MODEL_INTEL_BOOST",
        default=2,
        value_type="int",
        category="model",
        description="在请求复杂度基础上追加的智能等级",
        min_value=0,
        max_value=20,
        owner_module="core.model_provider",
    ),
    "model.reply_max_cost": SettingDef(
        key="model.reply_max_cost",
        env_name="REPLY_MODEL_MAX_COST",
        default=10.0,
        value_type="float",
        category="model",
        description="回复候选模型最大输入成本",
        min_value=0,
        max_value=1000,
        owner_module="core.model_provider",
    ),
    "prompt_runtime.engine": SettingDef(
        key="prompt_runtime.engine",
        env_name="NANOBOT_PROMPT_ENGINE",
        default="prompt",
        value_type="str",
        category="prompt",
        description="提示词运行引擎；旧 v1/v2 值会被迁移兼容层归一到 canonical runtime",
    ),
    ...
    "model.smart": SettingDef(
        key="model.smart",
        env_name="LLM_MODEL_SMART",
        default="",
        value_type="str",
        category="model",
        description="智能路由默认模型",
    ),
    "model.fast": SettingDef(
        key="model.fast",
        env_name="LLM_MODEL_FAST",
        default="",
        value_type="str",
        category="model",
        description="快速模型",
    ...
    "model.session_summary": SettingDef(
        key="model.session_summary",
        env_name="LLM_MODEL_SESSION_SUMMARY",
        default="",
        value_type="str",
        category="model",
        description="近期摘要模型",
    ...
    "model.route.timing_gate": SettingDef(
        key="model.route.timing_gate",
        env_name="CLASSIFIER_API_URL",
        default="http://172.17.0.1:9999/v1",
        value_type="str",
        category="model",
        description="分类器地址；完整配置请使用 model.route.timing_gate.model/api_key/timeout 等子字段",
    ),
    "model.route.timing_gate.model": SettingDef(
        key="model.route.timing_gate.model",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="TimingGate 模型名（API 调用时需要）",
    ),
    "model.route.timing_gate.api_key": SettingDef(
        key="model.route.timing_gate.api_key",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="TimingGate API key",
        sensitive=True,
    ),
    "model.route.timing_gate.timeout": SettingDef(
        key="model.route.timing_gate.timeout",
        env_name="",
        default=15,
        value_type="int",
        category="model",
        description="TimingGate 超时(秒)",
        min_value=3,
        max_value=120,
    ),
    "model.route.timing_gate.temperature": SettingDef(
        key="model.route.timing_gate.temperature",
        env_name="",
        default=0.0,
        value_type="float",
        category="model",
        description="TimingGate 温度",
        min_value=0,
        max_value=2,
    ),
    "model.route.timing_gate.max_tokens": SettingDef(
        key="model.route.timing_gate.max_tokens",
        env_name="",
        default=30,
        value_type="int",
        category="model",
        description="TimingGate 最大输出 tokens",
        min_value=5,
        max_value=500,
    ),
    "model.route.private_decision": SettingDef(
        key="model.route.private_decision",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="私聊决策分类器地址（空则回退到 timing_gate）",
    ),
    "model.route.private_decision.model": SettingDef(
        key="model.route.private_decision.model",
        env_name="",
        default="",
        value_type="str",
        category="model",
    ...
    "model.route.private_decision.api_key": SettingDef(
        key="model.route.private_decision.api_key",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="私聊决策 API key",
        sensitive=True,
    ),
    "model.route.private_decision.timeout": SettingDef(
        key="model.route.private_decision.timeout",
        env_name="",
        default=15,
        value_type="int",
        category="model",
    ...
    "model.route.private_decision.temperature": SettingDef(
        key="model.route.private_decision.temperature",
        env_name="",
        default=0.0,
        value_type="float",
        category="model",
    ...
    "model.route.private_decision.max_tokens": SettingDef(
        key="model.route.private_decision.max_tokens",
        env_name="",
        default=120,
        value_type="int",
        category="model",
    ...
    "model.route.classifier_legacy": SettingDef(
        key="model.route.classifier_legacy",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="旧Qwen分类器地址（空则回退到 timing_gate）",
    ),
    "model.route.classifier_legacy.model": SettingDef(
        key="model.route.classifier_legacy.model",
        env_name="",
        default="",
        value_type="str",
        category="model",
    ...
    "model.route.classifier_legacy.api_key": SettingDef(
        key="model.route.classifier_legacy.api_key",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="旧分类器 API key",
        sensitive=True,
    ),
    "model.route.classifier_legacy.timeout": SettingDef(
        key="model.route.classifier_legacy.timeout",
        env_name="",
        default=15,
        value_type="int",
        category="model",
    ...
    "model.route.classifier_legacy.temperature": SettingDef(
        key="model.route.classifier_legacy.temperature",
        env_name="",
        default=0.0,
        value_type="float",
        category="model",
    ...
    "model.route.classifier_legacy.max_tokens": SettingDef(
        key="model.route.classifier_legacy.max_tokens",
        env_name="",
        default=30,
        value_type="int",
        category="model",
    ...
    "model.route.sticker_describe": SettingDef(
        ...
        env_name="IMAGE_SUMMARY_API_URL",
        default="http://172.17.0.1:9999/v1",
        value_type="str",
        category="model",
        description="表情包打标 API 地址",
    ),
    "model.route.sticker_describe.model": SettingDef(
        key="model.route.sticker_describe.model",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="表情包打标模型名",
    ),
    "model.route.sticker_describe.api_key": SettingDef(
        key="model.route.sticker_describe.api_key",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="表情包打标 API key",
        sensitive=True,
    ),
    "model.route.sticker_describe.timeout": SettingDef(
        key="model.route.sticker_describe.timeout",
        env_name="",
        default=15,
        value_type="int",
        category="model",
    ...
    "model.route.sticker_describe.temperature": SettingDef(
        key="model.route.sticker_describe.temperature",
        env_name="",
        default=0.0,
        value_type="float",
        category="model",
    ...
    "model.route.sticker_describe.max_tokens": SettingDef(
        key="model.route.sticker_describe.max_tokens",
        env_name="",
        default=256,
        value_type="int",
        category="model",
    ...
    "rag.reranker.model_path": SettingDef(
        key="rag.reranker.model_path",
        env_name="RAG_LOCAL_RERANKER_MODEL",
        default="./models/bge-reranker-v2-m3",
        value_type="str",
        category="model",
        description="本地 RAG reranker 模型目录",
    ),
    "rag.reranker.hf_model": SettingDef(
        key="rag.reranker.hf_model",
        env_name="RAG_RERANKER_HF_MODEL",
        default="",
        value_type="str",
        category="model",
        description="可选 HuggingFace 下载源；默认目录使用 BAAI/bge-reranker-v2-m3",
    ),
    "rag.reranker.score_mode": SettingDef(
        key="rag.reranker.score_mode",
        env_name="RAG_RERANKER_SCORE_MODE",
        default="sigmoid",
        value_type="str",
        category="model",
        description="本地 RAG reranker 分数归一化: sigmoid/identity/minmax",
    ),
    "rag.reranker.max_text_chars": SettingDef(
        key="rag.reranker.max_text_chars",
        env_name="RAG_RERANKER_MAX_TEXT_CHARS",
        default=1200,
        value_type="int",
        category="model",
        description="发送给本地 reranker 的单候选文本最大字符数",
        min_value=100,
        max_value=20000,
    ),
    # ── Provider 供应商配置 ──
    "model.providers.newapi.base_url": SettingDef(
        key="model.providers.newapi.base_url",
        env_name="NEW_API_BASE_URL",
        default="",
        value_type="str",
        category="model",
        description="NewAPI 供应商地址",
    ),
    "model.providers.newapi.api_key": SettingDef(
        key="model.providers.newapi.api_key",
        env_name="NEW_API_KEY",
        default="",
        value_type="str",
        category="model",
        description="NewAPI API key",
        sensitive=True,
    ),
    "model.providers.newapi.enabled": SettingDef(
        key="model.providers.newapi.enabled",
        env_name="",
        default=True,
        value_type="bool",
        category="model",
        description="NewAPI 是否启用",
    ),
    "model.providers.newapi.registry_provider": SettingDef(
        key="model.providers.newapi.registry_provider",
        env_name="",
        default="new-api",
        value_type="str",
        category="model",
        description="NewAPI 在 registry 中的 provider 名",
    ),
    # [DEPRECATED] local_qwen is an alias — use local_llama instead
    "model.providers.local_qwen.base_url": SettingDef(
        key="model.providers.local_qwen.base_url",
        env_name="CLASSIFIER_API_URL",
        default="http://172.17.0.1:9999/v1",
        value_type="str",
        category="model",
        description="[DEPRECATED] 本地 Qwen 服务地址——请使用 local_llama",
    ),
    "model.providers.local_qwen.api_key": SettingDef(
        key="model.providers.local_qwen.api_key",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="[DEPRECATED] 本地 Qwen API key",
        sensitive=True,
    ),
    "model.providers.local_qwen.enabled": SettingDef(
        key="model.providers.local_qwen.enabled",
        env_name="",
        default=True,
        value_type="bool",
        category="model",
        description="[DEPRECATED] 本地 Qwen 是否启用",
    ),
    # [DEPRECATED] vision_qwen is an alias — use local_vision instead
    "model.providers.vision_qwen.base_url": SettingDef(
        key="model.providers.vision_qwen.base_url",
        env_name="IMAGE_SUMMARY_API_URL",
        default="http://172.17.0.1:9999/v1",
        value_type="str",
        category="model",
        description="[DEPRECATED] 视觉 Qwen 服务地址——请使用 local_vision",
    ),
    "model.providers.vision_qwen.api_key": SettingDef(
        key="model.providers.vision_qwen.api_key",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="[DEPRECATED] 视觉 Qwen API key",
        sensitive=True,
    ),
    "model.providers.vision_qwen.enabled": SettingDef(
        key="model.providers.vision_qwen.enabled",
        env_name="",
        default=True,
        value_type="bool",
        category="model",
        description="[DEPRECATED] 视觉 Qwen 是否启用",
    ),
    # ── 本地 llama.cpp 服务（原 local_qwen） ──
    "model.providers.local_llama.base_url": SettingDef(
        key="model.providers.local_llama.base_url",
        env_name="CLASSIFIER_API_URL",
        default="http://172.17.0.1:9999/v1",
        value_type="str",
        category="model",
        description="本地 llama.cpp 服务地址",
    ),
    "model.providers.local_llama.api_key": SettingDef(
        key="model.providers.local_llama.api_key",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="本地 llama.cpp API key",
        sensitive=True,
    ),
    "model.providers.local_llama.enabled": SettingDef(
        key="model.providers.local_llama.enabled",
        env_name="",
        default=True,
        value_type="bool",
        category="model",
        description="本地 llama.cpp 是否启用",
    ),
    "model.providers.local_llama.registry_provider": SettingDef(
        key="model.providers.local_llama.registry_provider",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="local_llama 在 registry 中的 provider 名",
    ),
    # ── 本地视觉模型（仅当 IMAGE_SUMMARY_API_URL != CLASSIFIER_API_URL 时才在列表中出现） ──
    "model.providers.local_vision.base_url": SettingDef(
        key="model.providers.local_vision.base_url",
        env_name="IMAGE_SUMMARY_API_URL",
        default="http://172.17.0.1:9999/v1",
        value_type="str",
        category="model",
        description="本地视觉模型服务地址（独立端点时启用）",
    ),
    "model.providers.local_vision.api_key": SettingDef(
        key="model.providers.local_vision.api_key",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="本地视觉模型 API key",
        sensitive=True,
    ),
    "model.providers.local_vision.enabled": SettingDef(
        key="model.providers.local_vision.enabled",
        env_name="",
        default=True,
        value_type="bool",
        category="model",
        description="本地视觉模型是否启用",
    ),
    "model.providers.local_vision.registry_provider": SettingDef(
        key="model.providers.local_vision.registry_provider",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="local_vision 在 registry 中的 provider 名",
    ),
    # ── Route provider 关联 ──
    "model.route.timing_gate.provider": SettingDef(
        ...
        value_type="str",
        category="model",
        description="TimingGate 使用的供应商",
    ),
    "model.route.private_decision.provider": SettingDef(
        key="model.route.private_decision.provider",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="私聊决策供应商（空=继承timing_gate）",
    ),
    "model.route.classifier_legacy.provider": SettingDef(
        key="model.route.classifier_legacy.provider",
        env_name="",
        default="",
        value_type="str",
        category="model",
        description="旧分类器供应商（空=继承timing_gate）",
    ),
    "model.route.sticker_describe.provider": SettingDef(
        ...
        value_type="str",
        category="model",
        description="图片描述供应商（默认与 timing_gate 共用同一端点）",
    ),
    "model.route.reply.provider": SettingDef(
        key="model.route.reply.provider",
        env_name="",
        default="newapi",
        value_type="str",
        category="model",
    ...
    "model.route.reply.timeout": SettingDef(
        key="model.route.reply.timeout",
        env_name="",
        default=120,
        value_type="int",
        category="model",
        description="主回复请求超时(秒)",
        min_value=3,
        max_value=300,
    ),
    "model.route.reply.temperature": SettingDef(
        key="model.route.reply.temperature",
        env_name="",
        default=0.7,
        value_type="float",
        category="model",
    ...
    "model.route.reply.max_tokens": SettingDef(
        key="model.route.reply.max_tokens",
        env_name="",
        default=0,
        value_type="int",
        category="model",
        description="主回复最大输出 tokens（0=由API决定）",
        min_value=0,
        max_value=200000,
    ),
    "model.route.fast.provider": SettingDef(
        key="model.route.fast.provider",
        env_name="",
        default="newapi",
        value_type="str",
        category="model",
    ...
    "model.route.fast.timeout": SettingDef(
        key="model.route.fast.timeout",
        env_name="",
        default=120,
        value_type="int",
        category="model",
        description="快速模型请求超时(秒)",
        min_value=3,
        max_value=300,
    ),
    "model.route.fast.temperature": SettingDef(
        key="model.route.fast.temperature",
        env_name="",
        default=0.7,
        value_type="float",
        category="model",
    ...
    "model.route.fast.max_tokens": SettingDef(
        key="model.route.fast.max_tokens",
        env_name="",
        default=0,
        value_type="int",
        category="model",
        description="快速模型最大输出 tokens（0=由API决定）",
        min_value=0,
        max_value=200000,
    ),
    "model.route.smart.provider": SettingDef(
        key="model.route.smart.provider",
        env_name="",
        default="newapi",
        value_type="str",
        category="model",
    ...
    "model.route.smart.timeout": SettingDef(
        key="model.route.smart.timeout",
        env_name="",
        default=120,
        value_type="int",
        category="model",
        description="智能模型请求超时(秒)",
        min_value=3,
        max_value=300,
    ),
    "model.route.smart.temperature": SettingDef(
        key="model.route.smart.temperature",
        env_name="",
        default=0.7,
        value_type="float",
        category="model",
    ...
    "model.route.smart.max_tokens": SettingDef(
        key="model.route.smart.max_tokens",
        env_name="",
        default=0,
        value_type="int",
        category="model",
        description="智能模型最大输出 tokens（0=由API决定）",
        min_value=0,
        max_value=200000,
    ),
    "model.route.session_summary.provider": SettingDef(
        key="model.route.session_summary.provider",
        env_name="",
        default="newapi",
        value_type="str",
        category="model",
    ...
    "model.route.session_summary.timeout": SettingDef(
        key="model.route.session_summary.timeout",
        env_name="",
        default=120,
        value_type="int",
        category="model",
        description="近期摘要请求超时(秒)",
        min_value=3,
        max_value=300,
    ),
    ...
    "model.route.session_summary.max_tokens": SettingDef(
        ...
        value_type="int",
        category="model",
        description="近期摘要最大输出 tokens",
        min_value=3000,
        max_value=12000,
    ),
    "model.route.memory_digest.provider": SettingDef(
        key="model.route.memory_digest.provider",
        env_name="",
        default="newapi",
        value_type="str",
        category="model",
    ...
    "model.route.memory_digest.timeout": SettingDef(
        key="model.route.memory_digest.timeout",
        env_name="",
        default=180,
        value_type="int",
        category="model",
        description="长期摘要请求超时(秒)",
        min_value=3,
        max_value=600,
    ),
    "model.route.memory_digest.temperature": SettingDef(
        key="model.route.memory_digest.temperature",
        env_name="",
        default=0.1,
        value_type="float",
        category="model",
    ...
    "model.route.memory_digest.max_tokens": SettingDef(
        key="model.route.memory_digest.max_tokens",
        env_name="",
        default=8192,
        value_type="int",
        category="model",
        description="长期摘要最大输出 tokens",
        min_value=4096,
        max_value=20000,
    ),
    "new_api.timeout": SettingDef(
        ...
        value_type="int",
        category="model",
        description="NewAPI超时(秒)",
        min_value=10,
        max_value=600,
    ),
    "new_api.max_retries": SettingDef(
        key="new_api.max_retries",
        env_name="NEW_API_MAX_RETRIES",
        default=3,
        value_type="int",
        category="model",
        description="NewAPI最大重试",
        min_value=0,
        max_value=10,
    ),
    "image_generation.model": SettingDef(
        key="image_generation.model",
        env_name="IMAGE_GENERATION_MODEL",
        default="gpt-image",
        value_type="str",
        category="image",
        description="图片生成模型",
    ...
    "image_generation.timeout": SettingDef(
        key="image_generation.timeout",
        env_name="IMAGE_GENERATION_TIMEOUT",
        default=600.0,
        value_type="float",
        category="image",
        description="图片生成超时(秒)",
        min_value=10,
        max_value=1200,
    ),
    "tool.lightweight_set": SettingDef(
        ...
        value_type="str",
        category="tool",
        description="显式 lightweight 兼容预设；普通聊天不自动使用（JSON数组）",
    ),
    "sandbox.enabled": SettingDef(
        key="sandbox.enabled",
        env_name="NANOBOT_SANDBOX_ENABLED",
        default=False,
        value_type="bool",
        category="sandbox",
        description="Sandbox 工具总开关",
        dangerous=True,
    ),
    ...
    "sandbox.session_execution_allowed": SettingDef(
        key="sandbox.session_execution_allowed",
        env_name="NANOBOT_SANDBOX_SESSION_EXECUTION_ALLOWED",
        default=False,
        value_type="bool",
        category="sandbox",
        description="宿主运维允许会话型 Sandbox 执行的安全硬上限",
        restart_required=True,
        dangerous=True,
        source_precedence=("environment", "default"),
        safety_class="invariant",
    ),
    "sandbox.developer_network_allowed": SettingDef(
        key="sandbox.developer_network_allowed",
        env_name="NANOBOT_SANDBOX_DEVELOPER_NETWORK_ALLOWED",
        default=False,
        value_type="bool",
        category="sandbox",
        description="宿主运维允许开发型 Sandbox 网络出口的安全硬上限",
        restart_required=True,
        dangerous=True,
        source_precedence=("environment", "default"),
        safety_class="invariant",
    ...
    "sandbox.exec_enabled": SettingDef(
        ...
        env_name="NANOBOT_SANDBOX_EXEC_ENABLED",
        default=False,
        value_type="bool",
        category="sandbox",
        description="Sandbox 命令执行能力开关",
        dangerous=True,
    ),
    "sandbox.group_enabled": SettingDef(
        key="sandbox.group_enabled",
        env_name="NANOBOT_SANDBOX_GROUP_ENABLED",
        default=False,
        value_type="bool",
        category="sandbox",
        description="群聊 Workspace 开关",
        dangerous=True,
    ),
    "sandbox.workspace_quota_bytes": SettingDef(
        key="sandbox.workspace_quota_bytes",
        env_name="NANOBOT_SANDBOX_WORKSPACE_QUOTA_BYTES",
        default=2 * 1024 * 1024 * 1024,
        value_type="int",
        category="sandbox",
        description="单 Workspace 逻辑配额（字节）",
        min_value=1024 * 1024,
        max_value=1024 * 1024 * 1024 * 1024,
    ),
    "sandbox.asset_max_bytes": SettingDef(
        key="sandbox.asset_max_bytes",
        env_name="NANOBOT_SANDBOX_ASSET_MAX_BYTES",
        default=512 * 1024 * 1024,
        value_type="int",
        category="sandbox",
        description="单个资产大小上限（字节）",
        min_value=1024,
        max_value=100 * 1024 * 1024 * 1024,
    ),
    "sandbox.total_quota_bytes": SettingDef(
        key="sandbox.total_quota_bytes",
        env_name="NANOBOT_SANDBOX_TOTAL_QUOTA_BYTES",
        default=10 * 1024 * 1024 * 1024,
        value_type="int",
        category="sandbox",
        description="Sandbox Workspace 总逻辑预算（字节）",
        min_value=1024 * 1024,
        max_value=10 * 1024 * 1024 * 1024 * 1024,
    ),
    "sandbox.disk_max_percent": SettingDef(
        key="sandbox.disk_max_percent",
        env_name="NANOBOT_SANDBOX_DISK_MAX_PERCENT",
        default=80,
        value_type="int",
        category="sandbox",
        description="拒绝新写入和执行的磁盘使用率水位",
        min_value=1,
        max_value=99,
    ),
    "sandbox.disk_min_free_bytes": SettingDef(
        key="sandbox.disk_min_free_bytes",
        env_name="NANOBOT_SANDBOX_DISK_MIN_FREE_BYTES",
        default=50 * 1024 * 1024 * 1024,
        value_type="int",
        category="sandbox",
        description="Sandbox 必须保留的最小可用磁盘空间（字节）",
        min_value=0,
        max_value=10 * 1024 * 1024 * 1024 * 1024,
    ),
    "sandbox.sandboxd_socket": SettingDef(
        key="sandbox.sandboxd_socket",
        env_name="NANOBOT_SANDBOXD_SOCKET",
        default="/run/nanobot-sandboxd/sandboxd.sock",
        value_type="str",
        category="sandbox",
        description="sandboxd Unix Socket 路径",
        restart_required=True,
    ),
    "sandbox.sandboxd_token_file": SettingDef(
        key="sandbox.sandboxd_token_file",
        env_name="NANOBOT_SANDBOXD_TOKEN_FILE",
        default="/run/nanobot-sandboxd/client.token",
        value_type="str",
        category="sandbox",
        description="sandboxd 客户端 Token 文件路径",
        restart_required=True,
    ),
    ...
    "sandbox.backend_timeout_seconds": SettingDef(
        key="sandbox.backend_timeout_seconds",
        env_name="NANOBOT_SANDBOX_BACKEND_TIMEOUT",
        default=15,
        value_type="int",
        category="sandbox",
        description="sandboxd 普通请求超时（秒）",
        min_value=1,
        max_value=60,
    ),
    "sandbox.run_timeout_seconds": SettingDef(
        key="sandbox.run_timeout_seconds",
        env_name="NANOBOT_SANDBOX_RUN_TIMEOUT",
        default=165,
        value_type="int",
        category="sandbox",
        description="sandboxd 执行请求客户端超时（秒）",
        min_value=10,
        max_value=180,
    ),
    "sandbox.lease_reconcile_interval_seconds": SettingDef(
        key="sandbox.lease_reconcile_interval_seconds",
        env_name="NANOBOT_SANDBOX_LEASE_RECONCILE_INTERVAL_SECONDS",
        default=15,
        value_type="int",
        category="sandbox",
        description="Server 主动拉取 Sandbox Lease 事实的周期（秒）",
        min_value=1,
        max_value=3600,
        restart_required=True,
    ),
    "sandbox.asset_transfer_timeout_seconds": SettingDef(
        ...
        value_type="int",
        category="sandbox",
        description="大资产经 Unix Socket 流式传输的超时（秒）",
        min_value=30,
        max_value=3600,
    ),
    "sandbox.asset_token_secret": SettingDef(
        key="sandbox.asset_token_secret",
        env_name="NANOBOT_ASSET_TOKEN_SECRET",
        default="",
        value_type="str",
        category="sandbox",
    ...
    "sandbox.asset_token_ttl_seconds": SettingDef(
        key="sandbox.asset_token_ttl_seconds",
        env_name="NANOBOT_ASSET_TOKEN_TTL_SECONDS",
        default=300,
        value_type="int",
        category="sandbox",
        description="资产下载 Token 默认有效期（秒）",
        min_value=60,
        max_value=86400,
    ),
    ...
    "router.cost_weight": SettingDef(
        key="router.cost_weight",
        env_name="ROUTER_COST_WEIGHT",
        default=6.0,
        value_type="float",
        category="router",
        description="路由成本权重",
    ...
    "router.intel_weight": SettingDef(
        key="router.intel_weight",
        env_name="ROUTER_INTEL_WEIGHT",
        default=5.0,
        value_type="float",
        category="router",
        description="路由智能度权重",
    ...
    "router.free_bonus": SettingDef(
        key="router.free_bonus",
        env_name="ROUTER_FREE_BONUS",
        default=-2.0,
        value_type="float",
        category="router",
        description="免费模型加分",
    ...
    "router.unstable_penalty": SettingDef(
        key="router.unstable_penalty",
        env_name="ROUTER_UNSTABLE_PENALTY",
        default=5.0,
        value_type="float",
        category="router",
        description="不稳定模型扣分",
    ...
    "model.max_consecutive_failures": SettingDef(
        ...
        value_type="int",
        category="router",
        description="熔断连续失败次数(需重建tracker)",
        min_value=1,
        max_value=20,
    ),
    "model.cooldown_base_seconds": SettingDef(
        ...
        value_type="int",
        category="router",
        description="熔断基础冷却(秒,需重建tracker)",
        min_value=30,
        max_value=3600,
    ),
    "model.cooldown_max_seconds": SettingDef(
        ...
        value_type="int",
        category="router",
        description="熔断最大冷却(秒,需重建tracker)",
        min_value=60,
        max_value=36000,
    ),
    "eval.sample_log_enabled": SettingDef(
        key="eval.sample_log_enabled",
        env_name="EVAL_SAMPLE_LOG_ENABLED",
        default=True,
        value_type="bool",
        category="eval",
    ...
    "eval.sample_db_enabled": SettingDef(
        key="eval.sample_db_enabled",
        env_name="EVAL_SAMPLE_DB_ENABLED",
        default=True,
        value_type="bool",
        category="eval",
    ...
    "eval.sample_interval_sec": SettingDef(
        key="eval.sample_interval_sec",
        env_name="EVAL_SAMPLE_INTERVAL_SEC",
        default=600,
        value_type="int",
        category="eval",
        description="自动采样间隔(秒)",
        min_value=60,
        max_value=3600,
    ),
    "eval.sample_limit_per_cycle": SettingDef(
        key="eval.sample_limit_per_cycle",
        env_name="EVAL_SAMPLE_LIMIT_PER_CYCLE",
        default=100,
        value_type="int",
        category="eval",
        description="每轮采样上限",
    ...
    "eval.log_path": SettingDef(
        key="eval.log_path",
        env_name="EVAL_LOG_PATH",
        default="data/nanobot.log",
        value_type="str",
        category="eval",
        description="采样日志路径",
    ),
    ...
    for _field, _default in _route_defaults.items():
        ...
        SETTING_DEFS.setdefault(
            ...
            SettingDef(
                ...
                else (3 if _field == "timeout" else 5),
                max_value=2
                if _field == "temperature"
                else (300 if _field == "timeout" else 65536),
            ),
        )
    SETTING_DEFS.setdefault(
...
def _validate_model_route_setting_specs() -> None:
    ...
    for descriptor in _MODEL_ROUTE_DESCRIPTORS:
        ...
        if (
            descriptor.execution_mode is not None
        ...
        expected_defaults = {
            ...
            f"{descriptor.setting_prefix}.max_tokens": (descriptor.default_max_tokens),
            f"{descriptor.setting_prefix}.enable_thinking": (
