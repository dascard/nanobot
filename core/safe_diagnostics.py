...
def _redact_structure(value: Any) -> Any:
    if isinstance(value, Mapping):
        return {
            str(key): "[REDACTED]" if _is_sensitive_key(key) else _redact_structure(item)
...
def safe_url_for_logging(value: object, *, max_chars: int = 2048) -> str:
    """返回可进入日志的 HTTP(S) URL，移除身份、敏感查询值和 fragment。"""
...
def safe_response_summary(
    ...
    *,
    ...
    """先脱敏再截断外部内容，并保证返回值不超过 ``max_chars``。"""
