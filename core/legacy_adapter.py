...
PERSONA_MERGE_PROMPT = """你是用户行为分析师。从对话日志中提取用户画像，帮助 AI 适配该用户。

核心原则：**只记人的偏好和判断。系统/代码/工具的事实不记——去日志里实时查。**
画像的目标不是描述用户，而是告诉 AI **应该怎么回应这个用户**。

## 输出 JSON
{
  "summary": "一句话（20-30字）",
  "traits": ["可观测行为模式，不是标签"],
  "preferences": ["用户明确或反复体现的偏好"],
  "pain_points": "观察到的用户受挫行为。描述用户做了什么/说了什么。无则空",
  "response_style": "AI 适配该用户时应采用的回复风格（语气、详略、要不要客套、接受什么格式等）。这是画像中最重要的字段",
  "domain_profiles": {"领域名": {"confidence": "high|medium|low", "summary": "简述", "interaction_count": 0}}
}

## 规则
- 连续出现 ≥2 次才写入。单次忽略
- 旧信号不再出现则降 confidence
- 忽略不携带特征的闲聊
"""

PERSONA_CRITIQUE_PROMPT = """你是画像质量审核员。审查候选画像，输出修正版。

## 五问审计
对每条 trait / preference / pain_point / response_style 描述：
1. **本能判定**：AI 不被告知也会这样做吗？ → 删
2. **冲突判定**：与画像中其他条目矛盾吗？ → 保留一方
3. **冗余判定**：换个说法重复了同一条信息吗？ → 合并
4. **创可贴判定**：是不是仅针对某一次翻车的临时修正？ → 删
5. **模糊判定**：每次读到它，理解会不一样吗？ → 改写或删

附加：描述的是人还是系统/工具？系统描述 → 删。

## 输出
先列修改要点（每条一行，<200字），再输出完整修正 JSON。
"""
