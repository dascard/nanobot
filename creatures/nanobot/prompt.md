...
1. **群聊分析/群日报/总结某群消息 → 直接调用 `group_analysis`**。不要先调用 `sql_analysis` 查群号、User 表或 ChatLog。`group_analysis` 的 `group_id` 可以是群号、`group_` 前缀 ID 或群名，工具内部会自动解析。
2. **`sql_analysis` 只在用户明确要求 SQL/查表/数据库审计/调试数据时使用**，不要作为任何业务工具的前置步骤。
3. 如果 `group_analysis` 返回"未找到群"或"多个候选群"，向用户澄清，不要擅自用 `sql_analysis` 继续探索。
